...
\newcommand{\mforward}{\cellcolor{teal!24}\rule{0pt}{1.5ex}\hspace{0.75em}}
\newcommand{\mgradq}{\cellcolor{red!24}\rule{0pt}{1.5ex}\hspace{0.75em}}
\newcommand{\mgradkv}{\cellcolor{yellow!35}\rule{0pt}{1.5ex}\hspace{0.75em}}
...
\subsection{Operational picture: what the kernels are actually doing}

The shortest faithful description is:
\begin{enumerate}[leftmargin=1.5em]
    \item start from one sparse support tile in the original coordinates;
    \item remove dead rows/columns and gather the surviving rows/columns into a
    smaller packed microproblem;
    \item run exact masked attention on that packed microproblem;
    \item scatter the output rows back, and in backward scatter
    \(d\widetilde Q\) while scatter-adding \(d\widetilde K,d\widetilde V\) into
    shared original key rows.
\end{enumerate}

Figure~\ref{fig:operational-packed-picture} makes this concrete. The packed
forward does \emph{not} approximate the softmax. It evaluates the exact masked
attention rule on a reindexed live submatrix, then maps the result back to the
original rows.
...
\centering
\textbf{1. Original sparse tile}\\[0.35em]
\begin{tabular}{c|*{6}{C}}
 & $k_1$ & $k_2$ & $k_3$ & $k_4$ & $k_5$ & $k_6$ \\
\hline
$q_1$ & \mlive & \mlive & \mempty & \mlive & \mempty & \mempty \\
$q_2$ & \mlive & \mlive & \mempty & \mlive & \mempty & \mempty \\
$q_3$ & \mempty & \mlive & \mempty & \mlive & \mlive & \mempty \\
$q_4$ & \mempty & \mlive & \mempty & \mempty & \mlive & \mempty \\
\end{tabular}

\vspace{0.35em}
\raggedright
\footnotesize
White cells are dead support. They never need scores, softmax mass, or value
lookups.
\end{minipage}
\hfill
\begin{minipage}[t]{0.31\textwidth}
\centering
\textbf{2. Packed forward microproblem}\\[0.35em]
\begin{tabular}{c|*{4}{C}}
 & $u_1{=}k_1$ & $u_2{=}k_2$ & $u_3{=}k_4$ & $u_4{=}k_5$ \\
\hline
$\tilde q_1{=}q_1$ & \mforward & \mforward & \mforward & \mempty \\
$\tilde q_2{=}q_2$ & \mforward & \mforward & \mforward & \mempty \\
$\tilde q_3{=}q_3$ & \mempty & \mforward & \mforward & \mforward \\
$\tilde q_4{=}q_4$ & \mempty & \mforward & \mempty & \mforward \\
\end{tabular}

\vspace{0.35em}
\raggedright
\footnotesize
Gather \(Q,K,V\) onto the live union columns only, keep the exact packed mask
\(\widetilde M\), and run exact masked attention there. The softmax domain is
the packed live support itself.
\end{minipage}
\hfill
\begin{minipage}[t]{0.31\textwidth}
\centering
\textbf{3. Backward writeback}\\[0.35em]
\begin{tabular}{c|*{4}{C}}
 & $q_1$ & $q_2$ & $q_3$ & $q_4$ \\
\hline
$dQ$ & \mgradq & \mgradq & \mgradq & \mgradq \\
\end{tabular}
...
\begin{tabular}{c|*{4}{C}}
 & $k_1$ & $k_2$ & $k_4$ & $k_5$ \\
\hline
$dK,dV$ & \mgradkv & \mgradkv & \mgradkv & \mgradkv \\
\end{tabular}

\vspace{0.35em}
\raggedright
\footnotesize
\(d\widetilde Q\) is row-owned and scatters cleanly. \(d\widetilde K\) and
\(d\widetilde V\) must scatter-add back into shared original key rows, which is
why backward batching and descriptor reuse matter much more than in forward.
\end{minipage}
\caption{Operational picture of packed HSA. Forward removes dead support,
reindexes the live rows/columns into a smaller exact masked microproblem, and
scatters the result back. Backward uses the same gather maps, but \(K/V\) are
shared across rows so the writeback is a reduction problem rather than a simple
row write.}
\label{fig:operational-packed-picture}
\end{figure}
...
One concrete improvement already landed early in this pass: sorting and
coalescing legacy packed backward panels by shape before execution. On a
representative Wikipedia $64$K batch, that reduced the hybrid backward batch
count from roughly
\[
...
\begin{enumerate}[leftmargin=1.5em]
    \item regroup \texttt{direct\_passthrough} families across qgroups instead
    of respecting qgroup boundaries too aggressively;
    \item regroup \texttt{k\_window} families across qgroups in the same way;
    \item add a real cached backward fast path for \texttt{masked\_union}
    payloads rather than falling back to legacy packed backward;
    \item increase the cached masked-forward range chunk size so very large
    \texttt{k\_window} ranges are not executed as dozens of tiny masked-kernel
    launches.
\end{enumerate}

The effect of the masked-union cached backward path is easy to see on single
batches:
\begin{center}
\begin{tabular}{lrr}
\toprule
...
\midrule
Wikipedia $64$K batch $0$ & $250.2$ ms & $34.3$ ms \\
FineWeb $128$K batch $0$ & $586.6$ ms & $106.0$ ms \\
\bottomrule
\end{tabular}
\end{center}

That eliminated the old backward-plumbing bottleneck. After that point, the
remaining wiki/web cost was mostly in packed forward geometry, especially on
large \texttt{k\_window} ranges.

\subsection{What the wiki/web geometry actually says}

Once the backward setup path was fixed, the next blocker was no longer generic
host-side sparse plumbing. It was the \emph{shape} of the packed problem on
wiki/web batches.

On a representative Wikipedia $64$K batch, the cached packed payload was:
\begin{itemize}[leftmargin=1.5em]
    \item total groups: \(8008\),
    \item \texttt{direct\_passthrough} groups: \(7756\),
    \item \texttt{k\_window} groups: \(28\),
    \item \texttt{union\_2d} groups: \(224\).
\end{itemize}

The more important number is the implied hardware area. On that same batch, the
direct family alone accounted for roughly
\[
881{,}966 / 896{,}751 \approx 98.3\%
\]
of the packed hardware area. So the practical problem was not ``a few awkward
tails.'' The practical problem was that the payload had been fragmented into
thousands of tiny direct groups.

This led to the current planner insight:
\begin{quote}
for these wiki/web batches, it is better to accept a slightly larger packed
union if it materially reduces group count and launch count.
\end{quote}

That is visible in the direct-family regroup experiment on the same Wikipedia
$64$K batch:
...
So the direct regrouping increases direct packed area by only about \(18\%\),
but reduces packed forward time by about \(2.44\times\). That is the missing
systems lesson: on non-Gutenberg geometry we were over-optimizing for local
packing density and under-optimizing for group granularity.

The same pattern reappeared once direct regrouping was fixed. On Wikipedia
\(64\)K, batches \(2\) through \(4\) were still dominated by huge
\texttt{k\_window} ranges. After the \texttt{k\_window} regrouping fix and the
masked-union cached backward path, those batches were no longer
backward-dominated; they were forward-dominated. The next winning change was to
raise the cached masked-forward range chunk size from \(128\) groups to
\(512\). On representative slow wiki batches this reduced forward time as
follows:
\begin{center}
\begin{tabular}{lrr}
\toprule
...
\midrule
Wikipedia $64$K batch $2$ & $149.0$ ms & $103.3$ ms \\
Wikipedia $64$K batch $4$ & $154.3$ ms & $112.5$ ms \\
FineWeb $128$K batch $0$ & $160.9$ ms & $109.8$ ms \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[leftmargin=1.5em]
    \item reduce direct fragmentation,
    \item reduce \texttt{k\_window} fragmentation,
    \item replace legacy packed backward with cached masked-union backward,
    \item then reduce forward launch overhead on the remaining large masked
    ranges.
\end{enumerate}

With those fixes in place, the representative non-Gutenberg resident runs now
look like this:
\begin{center}
\begin{tabular}{lrrr}
\toprule
...
\midrule
Wikipedia $64$K & flat FA4 & $143.5$ ms & $456.8$k \\
Wikipedia $64$K & HSA packed & $128.8$ ms & $509.0$k \\
FineWeb $128$K & flat FA4 & $476.2$ ms & $275.2$k \\
FineWeb $128$K & HSA packed & $188.3$ ms & $696.2$k \\
\bottomrule
\end{tabular}
\end{center}

That is the first point where the cross-dataset story becomes coherent:
Wikipedia \(64\)K now clears flat FA4, and FineWeb \(128\)K is well into the
regime where packed HSA is materially faster than flat.

\subsection{The apparent non-convergence issue was a benchmark LR mismatch}

One later scaled benchmark pass initially suggested that packed HSA was not
converging on Wikipedia and FineWeb, even though the step times were already in
the right systems regime. That turned out to be a harness issue, not an HSA
correctness failure: the hierarchical rows in that pass had been run with
\texttt{--hierarchical-lr-scale 0.1}, so the packed model was effectively
trained at one tenth of the baseline learning rate.

After rerunning the same resident benchmarks at
\texttt{hierarchical\_lr\_scale}=1.0, the packed rows returned to the same loss
regime as flat FA4:
\begin{center}
\begin{tabular}{lrrrr}
\toprule
...
\midrule
Wikipedia $131072$ & flat FA4 & $1.0$ & $3.3347$ & $276.2$k \\
Wikipedia $131072$ & HSA packed & $1.0$ & $3.3210$ & $854.9$k \\
Wikipedia $131072$ & strict\_log & $1.0$ & $3.3208$ & $1.527$M \\
FineWeb $262144$ & flat FA4 & $1.0$ & $3.3236$ & $151.1$k \\
FineWeb $262144$ & HSA packed & $1.0$ & $3.3011$ & $692.5$k \\
FineWeb $262144$ & strict\_log & $1.0$ & $3.2968$ & $1.586$M \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=1.5em]
    \item packed HSA does converge comparably to flat FA4 on these scaled
    wiki/web runs when trained at the same learning rate,
    \item the earlier poor packed-loss rows were an unfair benchmark setting,
    not evidence of a masked-union backward correctness problem,
    \item and the remaining competitive problem is the strength of
    \textbf{strict\_log}, not a flat-versus-packed convergence failure.
\end{itemize}

\subsection{Which knobs are actually worth tuning}

The current tuning picture is now clearer:
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{max\_rows\_per\_group}: the main granularity knob. Larger
    groups amortize launch and descriptor cost, but can increase union area.
    This is the first knob to push once fragmentation is the dominant problem.
    \item \texttt{max\_union\_k\_direct}: the ceiling for direct-family unions.
    If it is too small, direct work fragments; if it is too large, direct
    groups bloat and lose their advantage.
    \item \texttt{direct\_density\_threshold}: a secondary family-assignment
    knob. On the current Wikipedia \(64\)K slice, lowering it from \(0.85\) to
    \(0.55\) only moved runtime by about \(1.1\%\), so it is not the dominant
    lever.
    \item \texttt{window\_density\_threshold} and
    \texttt{k\_gap\_threshold}: route work between
    \texttt{direct\_passthrough}, \texttt{k\_window}, and \texttt{union\_2d}.
    They matter when families are misclassified, but they do not fix
    fragmentation by themselves.
    \item \texttt{packed\_forward\_block\_q}: q-side granularity. On the
    current Wikipedia \(64\)K benchmark, \(256\) outperformed \(128\).
    \item \texttt{tile\_k}: kernel tile width. This matters only after the
    group shapes are in a reasonable regime; it is not the first lever to pull
    when the payload is still exploding into tiny groups.
    \item \texttt{FLASH\_ATTN\_HSA\_CACHED\_MASKED\_GROUP\_CHUNK}: runtime
    chunking for large masked ranges. Once the payload has already been
    regrouped, this becomes the main forward-side launch-amortization knob for
    very large \texttt{k\_window} ranges. On the current wiki/web slices,
    \(512\) was clearly better than \(128\), while \(1024\) did not improve
    further.
\end{itemize}

So the practical tuning order is:
\begin{enumerate}[leftmargin=1.5em]
    \item reduce group fragmentation,
    \item then fix the backward path for the dominant residual family,
    \item then tune masked-range runtime chunking,
    \item only then tune direct-versus-union family assignment and tile-level
    kernel parameters.
\end{enumerate}
