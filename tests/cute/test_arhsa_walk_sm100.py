...
def test_arhsa_markov_incoming_step_matches_torch_reference():
    ...
    p_next_range = torch.full_like(p, 123.0)
    run_arhsa_markov_incoming_packed_range_step(
        p,
        incoming_edge_prob,
        incoming_src,
    ...
    torch.testing.assert_close(p_next_range[2:5], expected[2:5], atol=1e-6, rtol=1e-6)
    torch.testing.assert_close(p_next_range[:2], torch.full_like(p_next_range[:2], 123.0), atol=0, rtol=0)
    torch.testing.assert_close(p_next_range[5:], torch.full_like(p_next_range[5:], 123.0), atol=0, rtol=0)
...
def test_arhsa_leaf_readout_query_warp_matches_torch_reference(head_dim_v):
    ...
    expected_denom.index_add_(0, leaf_query_index, p[leaf_node_index].float())
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_direct_start_weighted_value_tensor_core_matches_reference():
    pytest.importorskip("cutlass")
    from flash_attn.cute.arhsa_walk_sm100 import (
        build_query_leaf_csr,
        run_arhsa_direct_start_weighted_value_tensor_core_forward,
    )
    ...
    head_dim = 64
    block_size = 16
    weights = torch.randn(19, n_heads, device=device, dtype=torch.bfloat16)
    rows = torch.tensor(
        [0, 2, 8, 1, 3, 5, 4, 6, 2, 0, 17, 19, 23, 31, 16, 18, 20, 22, 30],
        device=device,
        dtype=torch.int64,
    )
    queries = torch.tensor(
        [0, 0, 0, 1, 1, 1, 3, 3, 3, 4, 16, 16, 17, 17, 19, 21, 21, 30, 31],
        device=device,
        dtype=torch.int64,
    )
    value = torch.randn(n_outputs, n_heads, head_dim, device=device, dtype=torch.bfloat16)
    query_row_ptr, query_entry_index = build_query_leaf_csr(
        queries,
        n_queries=n_outputs,
    )

    got = run_arhsa_direct_start_weighted_value_tensor_core_forward(
        weights,
        value,
        rows,
        query_row_ptr,
...
def test_arhsa_leaf_readout_query_value_pack_matches_torch_reference():
    ...
    p = torch.rand(n_nodes, n_heads, device=device, dtype=torch.float32)
    value = torch.randn(leaves_per_query, n_heads, 64, device=device, dtype=torch.float32)
    query_leaf_row_ptr, query_leaf_entry_index = build_query_leaf_csr(
        leaf_query_index,
        n_queries=n_queries,
    )
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_leaf_readout_query_value_pack_tensor_core_matches_torch_bfloat16():
    pytest.importorskip("cutlass")
    ...
    leaf_node_index = torch.randint(n_nodes, (n_queries * leaves_per_query,), dtype=torch.int64, device=device)
    leaf_value_index = torch.arange(leaves_per_query, dtype=torch.int64, device=device).repeat(n_queries)
    p = torch.rand(n_nodes, n_heads, device=device, dtype=torch.bfloat16)
    value = torch.randn(leaves_per_query, n_heads, 64, device=device, dtype=torch.bfloat16)
    query_leaf_row_ptr, query_leaf_entry_index = build_query_leaf_csr(
        leaf_query_index,
        n_queries=n_queries,
    )
    ...
    got = run_arhsa_leaf_readout(
        ...
        denom=denom,
        query_value_pack=True,
        tensor_core_query_value_pack=True,
        pack_query_index=pack_query_index,
        pack_value_index=pack_value_index,
        pack_query_value_leaf_entry=pack_query_value_leaf_entry,
    )
    ...
    torch.testing.assert_close(denom, expected_denom.clamp(min=1e-8), atol=1e-3, rtol=1e-3)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_leaf_readout_query_value_pack16_tensor_core_matches_torch_bfloat16():
    pytest.importorskip("cutlass")
    if not torch.cuda.is_bf16_supported():
        pytest.skip("bfloat16 CUDA support required")
    from flash_attn.cute.arhsa_walk_sm100 import (
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_leaf_readout_backward_tensor_core_stats_matches_query_warp_bfloat16():
    pytest.importorskip("cutlass")
    if not torch.cuda.is_bf16_supported():
        pytest.skip("bfloat16 CUDA support required")
    from flash_attn.cute.arhsa_walk_sm100 import (
        build_query_leaf_csr,
        run_arhsa_leaf_readout_backward_stats_query_warp,
        run_arhsa_leaf_readout_backward_stats_tensor_core_d64,
    )
    ...
    leaf_node_index = torch.randint(n_nodes, (n_queries * leaves_per_query,), dtype=torch.int64, device=device)
    leaf_value_index = torch.randint(n_nodes, (n_queries * leaves_per_query,), dtype=torch.int64, device=device)
    value = torch.randn(n_nodes, n_heads, 64, device=device, dtype=torch.bfloat16)
    ...
    ref_leaf_grad_attn, ref_denom, ref_weighted = run_arhsa_leaf_readout_backward_stats_query_warp(
        p,
        leaf_node_index,
        leaf_value_index,
        query_leaf_row_ptr,
    ...
    got_leaf_grad_attn, got_denom, got_weighted = run_arhsa_leaf_readout_backward_stats_tensor_core_d64(
        p,
        leaf_node_index,
        leaf_value_index,
        query_leaf_row_ptr,
    ...
    torch.testing.assert_close(got_leaf_grad_attn, ref_leaf_grad_attn, atol=8e-2, rtol=8e-2)
    torch.testing.assert_close(got_denom, ref_denom, atol=2e-2, rtol=2e-2)
    torch.testing.assert_close(got_weighted, ref_weighted, atol=8e-2, rtol=8e-2)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_leaf_readout_backward_tensor_core_fused_matches_torch_bfloat16():
    pytest.importorskip("cutlass")
    if not torch.cuda.is_bf16_supported():
        pytest.skip("bfloat16 CUDA support required")
    from flash_attn.cute.arhsa_walk_sm100 import (
...
@pytest.mark.parametrize("scatter_mode", ["generic", "query_warp", "qv_pack", "qv_pack_tc_dv"])
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_leaf_readout_backward_tensor_core_query_value_packed_matches_torch_bfloat16(
    ...
    got_p, got_value = run_arhsa_leaf_readout_backward(
        ...
        query_value_pack_scatter=(scatter_mode in ("qv_pack", "qv_pack_tc_dv")),
        tensor_core_query_value_pack_scatter_dv=(scatter_mode == "qv_pack_tc_dv"),
        pack_query_index=pack_query_index,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_leaf_readout_backward_query_value_pack16_tensor_core_matches_torch_bfloat16():
    pytest.importorskip("cutlass")
    if not torch.cuda.is_bf16_supported():
        pytest.skip("bfloat16 CUDA support required")
    from flash_attn.cute.arhsa_walk_sm100 import (
    ...
    got_p, got_value = run_arhsa_leaf_readout_backward(
        ...
        n_queries=n_queries,
        tensor_core_query_value_packed=True,
        query_value_pack_scatter=True,
        tensor_core_query_value_pack_scatter_dv=True,
        pack_query_index=pack_query_index,
        pack_query_value_index=pack_query_value_index,
        pack_query_value_leaf_entry=pack_query_value_leaf_entry,
    )
...
def test_arhsa_walk_readout_autograd_backward_matches_torch_reference(incoming_packed_step, save_forward_history):
    ...
    torch.testing.assert_close(edge_scores_fast.grad, edge_scores_ref.grad, atol=1e-5, rtol=1e-5)
    torch.testing.assert_close(value_fast.grad, value_ref.grad, atol=1e-5, rtol=1e-5)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_arhsa_walk_readout_level_range_autograd_matches_torch_reference():
    pytest.importorskip("cutlass")
    from flash_attn.cute.arhsa_walk_sm100 import (
        arhsa_walk_readout_from_scores_fixed_iters_autograd,
        build_query_value_packs,
    ...
    device = torch.device("cuda")
    torch.manual_seed(61)
    src = torch.tensor([0, 0, 1, 1, 2, 2, 3, 3], dtype=torch.int32, device=device)
    dst = torch.tensor([2, 3, 2, 3, 4, 5, 4, 5], dtype=torch.int32, device=device)
    level_bounds = torch.tensor([0, 2, 4, 6], dtype=torch.int32)
    n_nodes = 6
    n_heads = 2
    n_queries = 2
    p0_data = torch.zeros(n_nodes, n_heads, device=device, dtype=torch.float32)
    p0_data[:2] = torch.rand(2, n_heads, device=device)
    edge_scores_data = torch.randn(src.numel(), n_heads, device=device, dtype=torch.float32)
    value_data = torch.randn(4, n_heads, 5, device=device, dtype=torch.float32)
    node_is_sink = torch.tensor([False, False, False, False, True, True], device=device)
    leaf_node_index = torch.tensor([4, 5, 4, 5], dtype=torch.int64, device=device)
    leaf_query_index = torch.tensor([0, 0, 1, 1], dtype=torch.int64, device=device)
    leaf_value_index = torch.tensor([0, 1, 2, 3], dtype=torch.int64, device=device)
    ...
    readout_fast = arhsa_walk_readout_from_scores_fixed_iters_autograd(
        ...
        n_queries=n_queries,
        n_iters=2,
        incoming_packed_step=True,
        save_forward_history=True,
        level_bounds=level_bounds,
        level_range_kernels=True,
    )
    ...
    torch.testing.assert_close(p0_fast.grad[:2], p0_ref.grad[:2], atol=1e-5, rtol=1e-5)
    torch.testing.assert_close(p0_fast.grad[2:], torch.zeros_like(p0_fast.grad[2:]), atol=0, rtol=0)
    torch.testing.assert_close(edge_scores_fast.grad, edge_scores_ref.grad, atol=1e-5, rtol=1e-5)
    torch.testing.assert_close(value_fast.grad, value_ref.grad, atol=1e-5, rtol=1e-5)
...
def test_arhsa_walk_readout_autograd_fast_query_warp_path_matches_torch_bfloat16():
    ...
    readout = arhsa_walk_readout_from_scores_fixed_iters_autograd(
        ...
        query_value_pack_readout=True,
        query_warp_scatter=True,
        query_warp_fused=True,
        save_forward_history=True,
        pack_query_index=pack_query_index,
        pack_query_value_index=pack_value_index,
        pack_query_value_leaf_entry=pack_query_value_leaf_entry,
    )
