...
class FlashAttentionBackwardSm100:
    ...
    def _setup_smem_layout(self):
        ...
        if const_expr(self.use_fp4_bwd_qk):
            ...
            sfdQ_a_layout = bs_layout.make_smem_layout_sfa(
                self.tiled_mma_dQ, self.mma_tiler_dsk, self.fp4_sf_vec_size, 1
            )
    ...
    def quantize_dS_fragment_to_fp4(
        ...
        logical_sfdQ_a = cute.make_tensor(
            ...
            bs_layout.tile_atom_to_shape_SF(
                (self.mma_tiler_dsk[0], self.mma_tiler_dsk[2], 1),
                self.fp4_sf_vec_size,
            ),
        )
    ...
    def kernel(
        ...
        if const_expr(self.use_2cta_instrs):
            ...
            sdS_xchg = None
        if const_expr(self.use_fp4_bwd_qk):
            sfdK_a = storage.sfdK_a.get_tensor(sfdK_a_layout)
            sfQcol = storage.sfQcol.get_tensor(sfQcol_layout)
            sfdQ_a = storage.sfdQ_a.get_tensor(sfdQ_a_layout)
            sfKcol = storage.sfKcol.get_tensor(sfKcol_layout)
        else:
        ...
        while work_tile.is_valid_tile:
            ...
            for iter_idx in cutlass.range(loop_count, unroll=1):
                ...
                for stage in cutlass.range_constexpr(num_stages):
                    ...
                    tdPrdS_r2t_f32 = None
                    if const_expr(self.use_fp4_bwd_qk):
                        tdPrdS_r2t_f32 = cute.make_fragment(tdPcdS_r2t[None, stage, 0, 0].shape, Float32)
