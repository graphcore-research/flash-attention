...
\subsection{Long-context caveat: the safe dense fallback is not a viable training path}

That pragmatic statement needs one important update. During the long-context
debugging pass we temporarily forced exact HSA backward to use the dense
\texttt{mask\_mod} traversal, because the legacy block-sparse backward had
earlier parity issues on short-context real-data runs. That was safe, but it
turned out to be unusable at real long sequence lengths.

The important measurement trap was that the original isolated
\texttt{fwdbwd} benchmark was profiling a degenerate backward regime. At step
$0$, the attention output projection \texttt{c\_proj} is zero-initialized, so
the upstream gradient flowing into the attention output is effectively zero.
Because the implementation has an exact zero-\texttt{dout} fast path, that
benchmark mostly measured:
\begin{itemize}[leftmargin=1.5em]
...
profiled step, the true long-context picture became obvious. On a resident real
Gutenberg batch at $T=262{,}144$:
\begin{itemize}[leftmargin=1.5em]
    \item dense-traversal fallback backward made the exact sparse path take on
    the order of tens of seconds per training step;
    \item re-enabling the legacy block-sparse traversal immediately removed that
    wall and brought the attention backward back down to the sub-second regime;
    \item for cached generalized forward with sparse backward, the post-update
    resident step is now approximately
    \[
    \text{forward} \approx 224\ \mathrm{ms}, \qquad
    \text{backward} \approx 176\ \mathrm{ms},
    \]
    so the packed method is no longer backward-bound.
\end{itemize}

This means the right conclusion is more precise than ``\texttt{sparse\_mask}
backward is always the robust choice.'' The intermediate conclusion during that
phase was:
\begin{quote}
For short contexts, dense traversal is a useful conservative fallback. For
long contexts, exact HSA training must use the block-sparse backward traversal,
otherwise the backward runtime dominates everything and the packed forward wins
never materialize end-to-end.
\end{quote}

That statement turned out to be incomplete. Once we ran fixed-seed
post-update parity checks on real Gutenberg caches, the picture became:
\begin{itemize}[leftmargin=1.5em]
    \item at $4096$, dense / sparse / cached still match after the first optimizer update;
    \item at $8192$, they still match;
    \item at $32768$, the block-sparse backward reintroduces a real sparse/cached
    mismatch on the first nonzero post-update step.
\end{itemize}
The decisive control is that forcing the dense traversal again at $32768$
collapses that mismatch back to BF16-scale noise. So the current correctness
policy is intentionally conservative: dense traversal is again the default for
multi-tile training, and block-sparse backward is opt-in until the long-context
nonzero-backward parity issue is fixed.

\subsection{What changed in the implementation}

The practical speedup came from a routing change, not from changing the HSA
algebra. The exact packed mask backward helper
\texttt{\_run\_hsa\_packed\_mask\_backward(...)} already knew how to call
FlashAttention backward with either:
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{block\_sparse\_tensors=None}, which forces dense traversal
    over the exact packed mask modifier; or
    \item a real \texttt{block\_sparse\_tensors} descriptor, which enables the
    legacy block-sparse traversal.
\end{itemize}

The key policy function is
\texttt{\_should\_disable\_hsa\_block\_sparse\_backward(...)}. It now does the
following:
\begin{enumerate}[leftmargin=1.5em]
    \item always disable block-sparse backward for single-tile cases;
    \item keep dense traversal by default for all multi-tile contexts;
    \item allow block-sparse backward only when the caller explicitly opts in,
    either with \texttt{FLASH\_ATTN\_HSA\_ENABLE\_BLOCK\_SPARSE\_BWD=1} or by
    setting an explicit \texttt{FLASH\_ATTN\_HSA\_BLOCK\_SPARSE\_BWD\_MIN\_SEQLEN};
    \item still allow explicit disable overrides for debugging.
\end{enumerate}

In other words, the earlier speedup came from stopping the long-context cached
training path from silently falling back to the dense traversal. The new
correctness result is that this is not yet a safe default. The block-sparse
backward remains the performance path we want, but it is currently a targeted
optimization path rather than the default training path.
