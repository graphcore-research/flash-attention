...
backward is always the robust choice.'' The correct statement is:
\begin{quote}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item keep dense traversal by default for short multi-tile contexts;
    \item default back to block-sparse traversal once sequence length is in the
    clearly long-context regime (currently guarded by a minimum sequence length
    threshold);
    \item still allow explicit overrides for debugging.
\end{enumerate}

In other words, the drastic speedup came from stopping the long-context cached
training path from silently falling back to the dense traversal. The backward
math did not suddenly become zero or trivial; we are simply running the right
kernel family again for the sequence-length regime that needs it.
