...
\emph{true multi-batch} resident training on real Gutenberg long-context
caches:
\begin{itemize}[leftmargin=1.5em]
    \item $64$K: $4$ distinct resident train batches;
    \item $128$K: $2$ distinct resident train batches;
...
\midrule
$65536$ & flat FA4 & $4$ & $2.7987$ & $143.1$ & $458.0$k \\
$65536$ & sliding\_log & $4$ & $2.1064$ & $60.6$ & $1082.1$k \\
$65536$ & sliding\_fixed & $4$ & $2.7185$ & $62.2$ & $1054.0$k \\
$65536$ & HSA packed & $4$ & $2.2469$ & $109.7$ & $597.2$k \\
\midrule
$131072$ & flat FA4 & $2$ & $2.8541$ & $581.4$ & $225.4$k \\
$131072$ & sliding\_log & $2$ & $1.6485$ & $158.3$ & $827.8$k \\
$131072$ & sliding\_fixed & $2$ & $2.7604$ & $167.8$ & $780.9$k \\
$131072$ & HSA packed & $2$ & $1.8864$ & $181.7$ & $721.3$k \\
\midrule
$262144$ & flat FA4 & $2$ & $2.8824$ & $1742.1$ & $150.5$k \\
$262144$ & sliding\_log & $2$ & $1.6699$ & $363.6$ & $721.0$k \\
$262144$ & sliding\_fixed & $2$ & $2.7400$ & $368.5$ & $711.3$k \\
$262144$ & HSA packed & $2$ & $1.8003$ & $344.5$ & $760.9$k \\
\bottomrule
\end{tabular}
\caption{True multi-batch resident training runs on real Gutenberg long-context
caches. These are still internal benchmark runs rather than full downstream
training jobs, but they are materially stronger than repeated-single-batch
smokes because they cycle over multiple distinct resident schedules.}
\label{tab:true-multibatch-resident}
\end{table}

Several points matter immediately.
\begin{enumerate}[leftmargin=1.5em]
    \item By $64$K the packed HSA path has already crossed the dense flat FA4
    baseline again after fixing the packed backward eligibility gate, and by
    $128$K and $262144$ it is comfortably faster than flat while also landing
    at lower loss on these short internal runs.
    ...
    surprisingly competitive in loss. That is a useful warning: any claim that
    HSA ``obviously'' dominates sliding windows needs to be supported by longer
    and less degenerate training, not by intuition alone.
    \item The earlier $64$K anomaly turned out to be a conservative fused
    backward eligibility gate, not a missing packed dispatch. One of the true
    multi-batch $64$K resident schedules had
    \texttt{fused\_total\_coverage\_frac} $\approx 0.99849$, which was below
    the previous $0.999$ cutoff and forced that batch onto the slow fallback
    route. Relaxing the packed benchmark path to a minimum fused-coverage
    threshold of $0.998$ restores the expected $64$K behavior.
\end{enumerate}

The $262144$ row is the first true multi-batch regime where the packed HSA path
cleanly clears the dense flat baseline on both throughput and loss while also
staying competitive with the stronger sliding-window baselines. That still does
not prove a general downstream quality win by itself, but it is the right
systems crossover: the packed hierarchical path is now winning in the regime it
was designed for, and the remaining work is to improve the smaller long-context
regimes and run longer multi-batch training.
