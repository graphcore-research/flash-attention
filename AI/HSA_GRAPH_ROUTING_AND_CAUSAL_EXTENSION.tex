...
$65536$ & HSA packed & $4$ & $2.2537$ & $2085.8$ & $31.4$k \\
\midrule
...
\begin{enumerate}[leftmargin=1.5em]
    \item By $128$K and $262144$, the packed HSA path is comfortably faster
    than flat dense FA4 while also landing at lower loss on these short
    internal runs.
    ...
    \item The $64$K packed row is currently anomalous. A direct profiler pass on
    the true multi-batch $64$K packed run shows that the packed forward and
    backward kernels are active and cheap, but the step is dominated by
    gather/scatter-style tensor plumbing (\texttt{nonzero}, \texttt{index},
    \texttt{gather}, \texttt{copy\_}) around the packed runtime on these more
    fragmented $64$K resident schedules. This is a real implementation issue,
