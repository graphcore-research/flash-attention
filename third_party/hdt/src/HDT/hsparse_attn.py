...
def _fwd_kernel(Q, K, V, hash_ids0, hash_ids1, hash_ids2,
                keep_ids0, keep_ids1, keep_ids2, anchor_pos, anchor_count,
    ...
    if min_q_hash != 1e9:
        ...
        for start_n in range(lo, hi):
            ...
            attn_mask = (~cls_mask) & ((qi[:, None] == 1) & (ki[None, :] == 1) & (qh_vals[:, None] == kh_vals[None, :]))
            # -- load k, v --
            ...
            offs_n += BLOCK_N
        batch_idx = off_hz // H
        anchor_limit = tl.load(anchor_count + batch_idx)
        anchor_base = 0
        anchor_offsets = batch_idx * stride_zid + tl.arange(0, BLOCK_N)
        anchor_tokens = tl.load(anchor_pos + anchor_offsets, mask=tl.arange(0, BLOCK_N) < anchor_limit, other=-1)
        offs_hash = (off_hz // H) * stride_zid + offs_m
        ...
        while anchor_base < anchor_limit:
            ...
            attn_mask = (
                ((qi1_vals[:, None] == 1) & (ki1_vals[None, :] == 1) & (qh1_vals[:, None] == kh1_vals[None, :]))
            ...
            m_i = m_i_new
            anchor_base += BLOCK_N
            anchor_offsets = batch_idx * stride_zid + anchor_base + tl.arange(0, BLOCK_N)
            anchor_tokens = tl.load(
                anchor_pos + anchor_offsets,
                mask=(anchor_base + tl.arange(0, BLOCK_N)) < anchor_limit,
                other=-1,
            )
    # write back l and m
    acc = acc / l_i[:, None]
...
def _bwd_kernel_one_col_block(Q, K, V, hash_ids0, hash_ids1, hash_ids2,
    ...
    anchor_limit = tl.load(anchor_count + batch_idx)
    anchor_base = 0
    anchor_offsets = batch_idx * stride_zid + tl.arange(0, BLOCK_M)
    anchor_tokens = tl.load(anchor_pos + anchor_offsets, mask=tl.arange(0, BLOCK_M) < anchor_limit, other=-1)
    offs_hash = (off_hz // H) * stride_zid + offs_n
    ...
    while anchor_base < anchor_limit:
        ...
        # increment pointers
        anchor_base += BLOCK_M
        anchor_offsets = batch_idx * stride_zid + anchor_base + tl.arange(0, BLOCK_M)
        anchor_tokens = tl.load(
            anchor_pos + anchor_offsets,
            mask=(anchor_base + tl.arange(0, BLOCK_M)) < anchor_limit,
            other=-1,
        )
    ...
    if not SEQUENCE_PARALLEL:
        for start_n in range(0, num_block_n):
            _bwd_kernel_one_col_block(Q, K, V, hash_ids0, hash_ids1, hash_ids2,
                                      keep_ids0, keep_ids1, keep_ids2, anchor_pos, anchor_count, sm_scale,
                                      qk_scale, Out, DO,  #
                                      DQ, DK, DV,  #
                                      L,  #
                                      D,  #
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, hash_ids, keep_ids, sm_scale, causal=False, output_attentions=False):
        ...
        anchor_lists = [anchor_ids[i].nonzero()[:, 0] for i in range(len(anchor_ids))]
        anchor_count = torch.tensor(
            [anchors.numel() for anchors in anchor_lists],
            device=q.device,
            dtype=torch.int32,
        )
        anchor_pos = torch.nested.to_padded_tensor(
            torch.nested.nested_tensor(anchor_lists),
            padding=-1,
            output_size=(q.shape[0], q.shape[2]),
        )
        if output_attentions:
        ...
        _fwd_kernel[grid](
            ...
            keep_ids[0], keep_ids[1], keep_ids[2], anchor_pos, anchor_count,
    ...
    def backward(ctx, do, A):
        ...
        BLOCK = 64
        q, k, v, o, L, hash_ids0, hash_ids1, hash_ids2, keep_ids0, keep_ids1, keep_ids2, anchor_pos, anchor_count = ctx.saved_tensors
        ...
        _bwd_kernel[(ctx.grid[1], triton.cdiv(seq_len_kv, BLOCK) if sequence_parallel else 1)](
            ...
            keep_ids0, keep_ids1, keep_ids2, anchor_pos, anchor_count, ctx.sm_scale,  #
            o, do,  #
            dq, dk, dv,  #
            L,  #
            delta,  #
