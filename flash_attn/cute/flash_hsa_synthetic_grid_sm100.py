...
class FlashHSACachedZeroThreeRowsSm100:
    ...
    def __call__(
        ...
        num_stream_rows = mRowIdx.shape[0]
        dq_elems_per_row = mdQRows.shape[1] * mdQRows.shape[2]
        dk_elems_per_row = mdKRows.shape[1] * mdKRows.shape[2]
        dv_elems_per_row = mdVRows.shape[1] * mdVRows.shape[2]
        elems_per_stream_row = dq_elems_per_row + dk_elems_per_row + dv_elems_per_row
        total_tasks = num_stream_rows * elems_per_stream_row
        grid_x = cute.ceil_div(total_tasks, self.num_threads)
        self.kernel(
            mRowIdx,
    ...
    def kernel(
        ...
        task_idx = block_idx * self.num_threads + tidx
        if task_idx < total_tasks:
            dq_elems_per_row = Int32(mdQRows.shape[1]) * Int32(mdQRows.shape[2])
            ...
            global_row = Int32(mRowIdx[stream_row])
            if global_row >= Int32(0):
                if rem < dq_elems_per_row:
                    head_idx = rem // Int32(mdQRows.shape[2])
                    dim_idx = rem - head_idx * Int32(mdQRows.shape[2])
                    mdQRows[global_row, head_idx, dim_idx] = Float32(0.0).to(mdQRows.element_type)
                elif rem < dq_elems_per_row + dk_elems_per_row:
                    local_rem = rem - dq_elems_per_row
                    head_idx = local_rem // Int32(mdKRows.shape[2])
                    dim_idx = local_rem - head_idx * Int32(mdKRows.shape[2])
                    mdKRows[global_row, head_idx, dim_idx] = Float32(0.0).to(mdKRows.element_type)
                else:
                    local_rem = rem - dq_elems_per_row - dk_elems_per_row
...
class FlashHSACachedCastThreeRowsSm100:
    ...
    def __call__(
        self,
        mdQSrcRows: cute.Tensor,
        mdKSrcRows: cute.Tensor,
        mdVSrcRows: cute.Tensor,
        mRowIdx: cute.Tensor,
        mdQDstRows: cute.Tensor,
        mdKDstRows: cute.Tensor,
        mdVDstRows: cute.Tensor,
        stream: cuda.CUstream,
    ):
        num_stream_rows = mRowIdx.shape[0]
        dq_elems_per_row = mdQSrcRows.shape[1] * mdQSrcRows.shape[2]
        dk_elems_per_row = mdKSrcRows.shape[1] * mdKSrcRows.shape[2]
        dv_elems_per_row = mdVSrcRows.shape[1] * mdVSrcRows.shape[2]
        elems_per_stream_row = dq_elems_per_row + dk_elems_per_row + dv_elems_per_row
        total_tasks = num_stream_rows * elems_per_stream_row
        grid_x = cute.ceil_div(total_tasks, self.num_threads)
        self.kernel(
    ...
    def kernel(
        ...
        task_idx = block_idx * self.num_threads + tidx
        if task_idx < total_tasks:
            dq_elems_per_row = Int32(mdQSrcRows.shape[1]) * Int32(mdQSrcRows.shape[2])
            dk_elems_per_row = Int32(mdKSrcRows.shape[1]) * Int32(mdKSrcRows.shape[2])
            dv_elems_per_row = Int32(mdVSrcRows.shape[1]) * Int32(mdVSrcRows.shape[2])
            elems_per_stream_row = dq_elems_per_row + dk_elems_per_row + dv_elems_per_row
            stream_row = task_idx // elems_per_stream_row
            rem = task_idx - stream_row * elems_per_stream_row
            global_row = Int32(mRowIdx[stream_row])
            if global_row >= Int32(0):
                if rem < dq_elems_per_row:
                    head_idx = rem // Int32(mdQSrcRows.shape[2])
                    dim_idx = rem - head_idx * Int32(mdQSrcRows.shape[2])
                    mdQDstRows[global_row, head_idx, dim_idx] = Float32(
                        mdQSrcRows[global_row, head_idx, dim_idx]
                    ).to(mdQDstRows.element_type)
                elif rem < dq_elems_per_row + dk_elems_per_row:
                    local_rem = rem - dq_elems_per_row
                    head_idx = local_rem // Int32(mdKSrcRows.shape[2])
                    dim_idx = local_rem - head_idx * Int32(mdKSrcRows.shape[2])
                    mdKDstRows[global_row, head_idx, dim_idx] = Float32(
                        mdKSrcRows[global_row, head_idx, dim_idx]
                    ).to(mdKDstRows.element_type)
                else:
                    local_rem = rem - dq_elems_per_row - dk_elems_per_row
                    head_idx = local_rem // Int32(mdVSrcRows.shape[2])
                    dim_idx = local_rem - head_idx * Int32(mdVSrcRows.shape[2])
                    mdVDstRows[global_row, head_idx, dim_idx] = Float32(
                        mdVSrcRows[global_row, head_idx, dim_idx]
                    ).to(mdVDstRows.element_type)


class FlashHSACachedCastTwoRowsSm100:
    """Cast DK/DV row accumulators into output tensors in one launch."""
    ...
    def kernel(
        ...
        if task_idx < total_tasks:
            ...
            if global_row >= Int32(0):
                if rem < dk_elems_per_row:
                    head_idx = rem // Int32(mdKSrcRows.shape[2])
                    dim_idx = rem - head_idx * Int32(mdKSrcRows.shape[2])
                    mdKDstRows[global_row, head_idx, dim_idx] = Float32(
                        mdKSrcRows[global_row, head_idx, dim_idx]
                    ).to(mdKDstRows.element_type)
                else:
...
class FlashHSASharedSupportBucketFwdSm100:
    """Benchmark-only shared-support forward kernel with one CTA per bucket/head."""

    arch = 100

    def __init__(self, *, qgroups_per_cta: int = 4):
        self.qgroups_per_cta = qgroups_per_cta
        # One warp per query row. Each qgroup contributes two packed rows.
        self.num_threads = 64 * qgroups_per_cta

    @cute.jit
    def __call__(
        self,
        mQPacked: cute.Tensor,
        mKShared: cute.Tensor,
        mVShared: cute.Tensor,
        mQLength: cute.Tensor,
        mMaskWords: cute.Tensor,
        softmax_scale: Float32,
        mOutPacked: cute.Tensor,
        ...
        grid_y = mQPacked.shape[2]
        grid_x = (mQPacked.shape[0] + self.qgroups_per_cta - 1) // self.qgroups_per_cta
        self.kernel(
            mQPacked,
            mKShared,
            mVShared,
    ...
    def kernel(
        ...
        tidx, _, _ = cute.arch.thread_idx()
        qgroup_tile_idx, head_idx, _ = cute.arch.block_idx()
        warp_idx = tidx // cute.arch.WARP_SIZE
        qgroup_in_cta = warp_idx // Int32(2)
        row_idx = warp_idx % Int32(2)
        lane = tidx % cute.arch.WARP_SIZE
        qgroup_base = qgroup_tile_idx * Int32(self.qgroups_per_cta)
        qgroup_count = Int32(mQPacked.shape[0])
        smem = cutlass.utils.SmemAllocator()
        sQ = smem.allocate_tensor(mQPacked.element_type, cute.make_layout((self.qgroups_per_cta * 2, 64)), byte_alignment=16)
        sK = smem.allocate_tensor(mKShared.element_type, cute.make_layout((32, 64)), byte_alignment=16)
        sV = smem.allocate_tensor(mVShared.element_type, cute.make_layout((32, 64)), byte_alignment=16)
        dim0 = lane
        dim1 = lane + cute.arch.WARP_SIZE

        for elem_idx in cutlass.range(
            tidx,
            Int32(self.qgroups_per_cta) * Int32(2) * Int32(64),
            self.num_threads,
            unroll=1,
        ):
            row_load = elem_idx // Int32(64)
            dim_idx = elem_idx - row_load * Int32(64)
            qgroup_load = row_load // Int32(2)
            qrow_load = row_load % Int32(2)
            qgroup_idx = qgroup_base + qgroup_load
            if qgroup_idx < qgroup_count:
                q_length = Int32(mQLength[qgroup_idx])
                if qrow_load < q_length:
                    sQ[row_load, dim_idx] = mQPacked[qgroup_idx, qrow_load, head_idx, dim_idx]
                else:
                    sQ[row_load, dim_idx] = Float32(0.0).to(sQ.element_type)
            else:
                sQ[row_load, dim_idx] = Float32(0.0).to(sQ.element_type)
        cute.arch.barrier()
        ...
        if qgroup_valid:
            q_length = Int32(mQLength[qgroup_idx])
        active_row = qgroup_valid and row_idx < q_length
        q_row_slot = qgroup_in_cta * Int32(2) + row_idx

        row_max = -Float32.inf
        row_sum = Float32(0.0)
        out0 = Float32(0.0)
        out1 = Float32(0.0)
        support_rows = Int32(mKShared.shape[0])

        for tile_start in range(0, mKShared.shape[0], cute.arch.WARP_SIZE):
            for elem_idx in cutlass.range(tidx, cute.arch.WARP_SIZE * Int32(64), self.num_threads, unroll=1):
                tile_slot = elem_idx // Int32(64)
                dim_idx = elem_idx - tile_slot * Int32(64)
                global_key = Int32(tile_start) + tile_slot
                if global_key < support_rows:
                    sK[tile_slot, dim_idx] = mKShared[global_key, head_idx, dim_idx]
                    sV[tile_slot, dim_idx] = mVShared[global_key, head_idx, dim_idx]
                else:
                    sK[tile_slot, dim_idx] = Float32(0.0).to(sK.element_type)
                    sV[tile_slot, dim_idx] = Float32(0.0).to(sV.element_type)
            cute.arch.barrier()

            if warp_idx < Int32(self.qgroups_per_cta * 2):
                global_key = Int32(tile_start) + lane
                score = -Float32.inf
                key_valid = Boolean(False)
                if active_row and global_key < support_rows:
                    ...
                    if key_valid:
                        ...
                        for dim_idx in range(0, 64, 4):
                            score += Float32(sQ[q_row_slot, dim_idx + 0]) * Float32(sK[lane, dim_idx + 0])
                            score += Float32(sQ[q_row_slot, dim_idx + 1]) * Float32(sK[lane, dim_idx + 1])
                            score += Float32(sQ[q_row_slot, dim_idx + 2]) * Float32(sK[lane, dim_idx + 2])
                            score += Float32(sQ[q_row_slot, dim_idx + 3]) * Float32(sK[lane, dim_idx + 3])
                        score *= softmax_scale
        ...
        if warp_idx < Int32(self.qgroups_per_cta * 2):
            if active_row:
                if row_max == -Float32.inf or row_sum == Float32(0.0):
                    ...
                    if dim0 < mOutPacked.shape[3]:
                        mOutPacked[qgroup_idx, row_idx, head_idx, dim0] = (out0 * inv_row_sum).to(mOutPacked.element_type)
                    if dim1 < mOutPacked.shape[3]:
                        mOutPacked[qgroup_idx, row_idx, head_idx, dim1] = (out1 * inv_row_sum).to(mOutPacked.element_type)
                    if lane == Int32(0):
                        mLSEPacked[qgroup_idx, row_idx, head_idx] = (
...
class FlashHSASharedSupportBucketFwdWide64Sm100:
    """Benchmark-only shared-support forward kernel with 64-key chunks and two warps per row."""

    arch = 100

    def __init__(self, *, qgroups_per_cta: int = 2):
        self.qgroups_per_cta = qgroups_per_cta
        # Each qgroup contributes 2 rows, and each row uses 2 warps to cover 64 keys per chunk.
        self.num_threads = 128 * qgroups_per_cta
    ...
    def kernel(
        ...
        qgroup_tile_idx, head_idx, _ = cute.arch.block_idx()
        warp_idx = tidx // cute.arch.WARP_SIZE
        warp_in_qgroup = warp_idx % Int32(4)
        qgroup_in_cta = warp_idx // Int32(4)
        row_idx = warp_in_qgroup // Int32(2)
        half_idx = warp_in_qgroup % Int32(2)
        lane = tidx % cute.arch.WARP_SIZE
        qgroup_base = qgroup_tile_idx * Int32(self.qgroups_per_cta)
        qgroup_count = Int32(mQPacked.shape[0])
        smem = cutlass.utils.SmemAllocator()
        sQ = smem.allocate_tensor(
            mQPacked.element_type, cute.make_layout((self.qgroups_per_cta * 2, 64)), byte_alignment=16
        )
        sK = smem.allocate_tensor(mKShared.element_type, cute.make_layout((64, 64)), byte_alignment=16)
        sV = smem.allocate_tensor(mVShared.element_type, cute.make_layout((64, 64)), byte_alignment=16)
        sChunkMax = smem.allocate_tensor(cutlass.Float32, cute.make_layout((self.qgroups_per_cta * 2, 2)), byte_alignment=16)
        sChunkSum = smem.allocate_tensor(cutlass.Float32, cute.make_layout((self.qgroups_per_cta * 2, 2)), byte_alignment=16)
        sChunkOut0 = smem.allocate_tensor(
            cutlass.Float32,
            cute.make_layout((self.qgroups_per_cta * 2, 2, 32)),
            byte_alignment=16,
        )
        sChunkOut1 = smem.allocate_tensor(
            cutlass.Float32,
            cute.make_layout((self.qgroups_per_cta * 2, 2, 32)),
            byte_alignment=16,
        )
        dim0 = lane
        dim1 = lane + cute.arch.WARP_SIZE
        row_slot = qgroup_in_cta * Int32(2) + row_idx
        ...
        for elem_idx in cutlass.range(
            ...
            q_row_slot = elem_idx // Int32(64)
            dim_idx = elem_idx - q_row_slot * Int32(64)
            qgroup_load = q_row_slot // Int32(2)
            qrow_load = q_row_slot % Int32(2)
            qgroup_idx = qgroup_base + qgroup_load
            if qgroup_idx < qgroup_count:
                q_length = Int32(mQLength[qgroup_idx])
                if qrow_load < q_length:
                    sQ[q_row_slot, dim_idx] = mQPacked[qgroup_idx, qrow_load, head_idx, dim_idx]
                else:
                    sQ[q_row_slot, dim_idx] = Float32(0.0).to(sQ.element_type)
            else:
                sQ[q_row_slot, dim_idx] = Float32(0.0).to(sQ.element_type)
        cute.arch.barrier()
        ...
        active_row = qgroup_valid and row_idx < q_length
        support_rows = Int32(mKShared.shape[0])
        row_owner = half_idx == Int32(0)
        row_max = -Float32.inf
        row_sum = Float32(0.0)
        out0 = Float32(0.0)
        out1 = Float32(0.0)

        for tile_start in range(0, mKShared.shape[0], 64):
            for elem_idx in cutlass.range(tidx, Int32(64) * Int32(64), self.num_threads, unroll=1):
                tile_slot = elem_idx // Int32(64)
                dim_idx = elem_idx - tile_slot * Int32(64)
                global_key = Int32(tile_start) + tile_slot
                if global_key < support_rows:
            ...
            global_key = Int32(tile_start) + half_idx * Int32(32) + lane
            score = -Float32.inf
            key_valid = Boolean(False)
            if active_row and global_key < support_rows:
                word_idx = global_key // Int32(32)
                ...
                if key_valid:
                    score = Float32(0.0)
                    for dim_idx in range(0, 64, 4):
                        score += Float32(sQ[row_slot, dim_idx + 0]) * Float32(sK[half_idx * Int32(32) + lane, dim_idx + 0])
                        score += Float32(sQ[row_slot, dim_idx + 1]) * Float32(sK[half_idx * Int32(32) + lane, dim_idx + 1])
                        score += Float32(sQ[row_slot, dim_idx + 2]) * Float32(sK[half_idx * Int32(32) + lane, dim_idx + 2])
                        score += Float32(sQ[row_slot, dim_idx + 3]) * Float32(sK[half_idx * Int32(32) + lane, dim_idx + 3])
                    score *= softmax_scale
            local_max = utils.warp_reduce(score, utils.fmax)
            if lane == Int32(0):
                sChunkMax[row_slot, half_idx] = local_max
            cute.arch.barrier()

            chunk_max = sChunkMax[row_slot, Int32(0)]
            other_max = sChunkMax[row_slot, Int32(1)]
            if other_max > chunk_max:
                chunk_max = other_max
            prob = Float32(0.0)
            if key_valid and chunk_max != -Float32.inf:
                prob = cute.math.exp2((score - chunk_max) * Float32(_LOG2_E), fastmath=True)
            local_sum = utils.warp_reduce(prob, lambda a, b: a + b)
            chunk_out0 = Float32(0.0)
            chunk_out1 = Float32(0.0)
            for k_rel in range(cute.arch.WARP_SIZE):
                prob_k = utils.shuffle_sync(prob, k_rel)
                global_slot = half_idx * Int32(32) + Int32(k_rel)
                if dim0 < mOutPacked.shape[3]:
                    chunk_out0 += prob_k * Float32(sV[global_slot, dim0])
                if dim1 < mOutPacked.shape[3]:
                    chunk_out1 += prob_k * Float32(sV[global_slot, dim1])
            if lane == Int32(0):
                sChunkSum[row_slot, half_idx] = local_sum
            sChunkOut0[row_slot, half_idx, lane] = chunk_out0
            sChunkOut1[row_slot, half_idx, lane] = chunk_out1
            cute.arch.barrier()

            if row_owner and chunk_max != -Float32.inf:
                chunk_sum = sChunkSum[row_slot, Int32(0)] + sChunkSum[row_slot, Int32(1)]
                next_max = row_max if row_max > chunk_max else chunk_max
                prev_scale = Float32(0.0) if row_max == -Float32.inf else cute.math.exp2(
                    (row_max - next_max) * Float32(_LOG2_E), fastmath=True
                )
                chunk_scale = cute.math.exp2((chunk_max - next_max) * Float32(_LOG2_E), fastmath=True)
                out0 = out0 * prev_scale + (
                    sChunkOut0[row_slot, Int32(0), lane] + sChunkOut0[row_slot, Int32(1), lane]
                ) * chunk_scale
                out1 = out1 * prev_scale + (
                    sChunkOut1[row_slot, Int32(0), lane] + sChunkOut1[row_slot, Int32(1), lane]
                ) * chunk_scale
                row_sum = row_sum * prev_scale + chunk_sum * chunk_scale
                row_max = next_max
            cute.arch.barrier()
...
class FlashHSASynthetic2DMaskedGatherScatterFwdSm100:
    """Benchmark-only masked forward that gathers Q/K/V rows and writes final rows directly."""
...
class FlashHSASynthetic2DMaskedGatherCombineFwdSm100:
    """Masked gather forward that online-combines rows into an existing FP32 destination."""
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.rows_per_cta):
            ...
            if active_row:
                row_lse = -Float32.inf
                row_out0 = Float32(0.0)
                row_out1 = Float32(0.0)
                if row_max != -Float32.inf and row_sum != Float32(0.0):
                    inv_row_sum = Float32(1.0) / row_sum
                    row_out0 = out0 * inv_row_sum
                    row_out1 = out1 * inv_row_sum
                    row_lse = row_max + ssa_to_scalar(
                        cute.math.log(scalar_to_ssa(row_sum, Float32), fastmath=True)
                    )
                prev_lse = Float32(mLSERows[global_q_row, head_idx])
                max_lse = prev_lse if prev_lse > row_lse else row_lse
                next_lse = -Float32.inf
                prev_weight = Float32(0.0)
                row_weight = Float32(0.0)
                if max_lse != -Float32.inf:
                    ...
                    next_lse = ssa_to_scalar(cute.math.log(scalar_to_ssa(sum_scale, Float32), fastmath=True)) + max_lse
                    inv_sum = Float32(0.0) if (sum_scale == Float32(0.0) or sum_scale != sum_scale) else Float32(1.0) / sum_scale
                    prev_weight = prev_scale * inv_sum
                    row_weight = row_scale * inv_sum
                if dim0 < mOutRows.shape[2]:
                    prev_out0 = Float32(mOutRows[global_q_row, head_idx, dim0])
                    mOutRows[global_q_row, head_idx, dim0] = (prev_out0 * prev_weight + row_out0 * row_weight).to(
                        mOutRows.element_type
                    )
                if dim1 < mOutRows.shape[2]:
                    prev_out1 = Float32(mOutRows[global_q_row, head_idx, dim1])
                    mOutRows[global_q_row, head_idx, dim1] = (prev_out1 * prev_weight + row_out1 * row_weight).to(
                        mOutRows.element_type
                    )
                if lane == Int32(0):
                    mLSERows[global_q_row, head_idx] = next_lse.to(mLSERows.element_type)


class FlashHSASynthetic2DMaskedFwdD128Sm100:
    """Masked packed forward for 128-wide Q/K/V buckets."""
    ...
    def kernel(
        ...
        sQ = smem.allocate_tensor(
            mQPacked.element_type,
            cute.make_layout((self.rows_per_cta, 128)),
            byte_alignment=16,
        )
        sK = smem.allocate_tensor(
            mKPacked.element_type,
            cute.make_layout((self.tile_k, 128)),
            byte_alignment=16,
        )
        sV = smem.allocate_tensor(
            mVPacked.element_type,
        ...
        dim0 = lane
        dim1 = lane + cute.arch.WARP_SIZE
        dim2 = lane + cute.arch.WARP_SIZE * Int32(2)
        dim3 = lane + cute.arch.WARP_SIZE * Int32(3)

        q_length = Int32(mQLength[bucket_idx])
        for elem_idx in cutlass.range(tidx, Int32(self.rows_per_cta) * Int32(128), self.num_threads, unroll=1):
        ...
        if warp_idx < Int32(self.rows_per_cta):
            ...
            for tile_start in range(0, mKPacked.shape[1], self.tile_k):
                for elem_idx in cutlass.range(tidx, Int32(self.tile_k) * Int32(128), self.num_threads, unroll=1):
                ...
                if chunk_max != -Float32.inf:
                    ...
                    for k_rel in range(cute.arch.WARP_SIZE):
                        ...
                        if dim1 < mOutPacked.shape[3]:
                            chunk_out1 += prob_k * Float32(sV[k_rel, dim1])
                        if dim2 < mOutPacked.shape[3]:
                            chunk_out2 += prob_k * Float32(sV[k_rel, dim2])
                        if dim3 < mOutPacked.shape[3]:
            ...
            if active_row:
                if row_max == -Float32.inf or row_sum == Float32(0.0):
                    ...
                    if dim1 < mOutPacked.shape[3]:
                        mOutPacked[bucket_idx, row_idx, head_idx, dim1] = Float32(0.0).to(mOutPacked.element_type)
                    if dim2 < mOutPacked.shape[3]:
                        mOutPacked[bucket_idx, row_idx, head_idx, dim2] = Float32(0.0).to(mOutPacked.element_type)
                    if dim3 < mOutPacked.shape[3]:
                        mOutPacked[bucket_idx, row_idx, head_idx, dim3] = Float32(0.0).to(mOutPacked.element_type)
                else:
                    inv_row_sum = Float32(1.0) / row_sum
                    if dim0 < mOutPacked.shape[3]:
                        mOutPacked[bucket_idx, row_idx, head_idx, dim0] = (out0 * inv_row_sum).to(mOutPacked.element_type)
                    if dim1 < mOutPacked.shape[3]:
                        mOutPacked[bucket_idx, row_idx, head_idx, dim1] = (out1 * inv_row_sum).to(mOutPacked.element_type)
                    if dim2 < mOutPacked.shape[3]:
                        mOutPacked[bucket_idx, row_idx, head_idx, dim2] = (out2 * inv_row_sum).to(mOutPacked.element_type)
                    if dim3 < mOutPacked.shape[3]:
                        mOutPacked[bucket_idx, row_idx, head_idx, dim3] = (out3 * inv_row_sum).to(mOutPacked.element_type)
                    if lane == Int32(0):
                        mLSEPacked[bucket_idx, row_idx, head_idx] = (
                            row_max + ssa_to_scalar(cute.math.log(scalar_to_ssa(row_sum, Float32), fastmath=True))
                        ).to(mLSEPacked.element_type)
...
class FlashHSASynthetic2DMaskedGatherFwdD128Sm100:
    """Masked forward that gathers 128-wide Q/K/V rows in-kernel."""
...
class FlashHSASynthetic2DMaskedGatherScatterFwdD128Sm100:
    """Masked gather+scatter forward for 128-wide Q/K/V rows."""
...
class FlashHSASynthetic2DMaskedGatherCombineFwdD128Sm100:
    """Masked gather forward that online-combines 128-wide rows into an existing FP32 destination."""
    ...
    def kernel(
        ...
        if warp_idx < Int32(self.rows_per_cta):
            ...
            if active_row:
                ...
                row_out0 = Float32(0.0)
                row_out1 = Float32(0.0)
                row_out2 = Float32(0.0)
                row_out3 = Float32(0.0)
                if row_max != -Float32.inf and row_sum != Float32(0.0):
                    inv_row_sum = Float32(1.0) / row_sum
                    row_out0 = out0 * inv_row_sum
                    row_out1 = out1 * inv_row_sum
                    row_out2 = out2 * inv_row_sum
                    row_out3 = out3 * inv_row_sum
                    row_lse = row_max + ssa_to_scalar(
                        cute.math.log(scalar_to_ssa(row_sum, Float32), fastmath=True)
                    )
                prev_lse = Float32(mLSERows[global_q_row, head_idx])
                ...
                if dim2 < mOutRows.shape[2]:
                    prev_out2 = Float32(mOutRows[global_q_row, head_idx, dim2])
                    mOutRows[global_q_row, head_idx, dim2] = (prev_out2 * prev_weight + row_out2 * row_weight).to(
                        mOutRows.element_type
                    )
                if dim3 < mOutRows.shape[2]:
                    prev_out3 = Float32(mOutRows[global_q_row, head_idx, dim3])
                    mOutRows[global_q_row, head_idx, dim3] = (prev_out3 * prev_weight + row_out3 * row_weight).to(
                        mOutRows.element_type
                    )
                if lane == Int32(0):
                    mLSERows[global_q_row, head_idx] = next_lse.to(mLSERows.element_type)
...
class FlashHSASynthetic2DMaskedGatherScatterTCFwdSm100:
    """Benchmark-only gather+scatter forward with TC score tiles for 16x32 union buckets."""

    arch = 100

    def __init__(self, *, rows_per_cta: int = 16, tile_k: int = 32):
        if rows_per_cta != 16:
            raise ValueError("FlashHSASynthetic2DMaskedGatherScatterTCFwdSm100 requires rows_per_cta=16")
        if tile_k != 32:
            raise ValueError("FlashHSASynthetic2DMaskedGatherScatterTCFwdSm100 requires tile_k=32")
        self.rows_per_cta = rows_per_cta
        self.tile_k = tile_k
        self.num_threads = 32
    ...
    def kernel(
        ...
        tile32x64_layout = cute.tile_to_shape(
            sm80_utils.get_smem_layout_atom(mVRows.element_type, 64),
            (32, 64),
            (0, 1),
        )
        sQ = smem.allocate_tensor(
        ...
        for tile_start in range(0, mKRowIdx.shape[1], 32):
            for elem_idx in cutlass.range(lane, Int32(32) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
                tile_row = elem_idx // Int32(64)
                dim_idx = elem_idx - tile_row * Int32(64)
                global_key = Int32(tile_start) + tile_row
...
class FlashHSASynthetic2DExactGatherScatterTCFwdSm100:
    ...
    def __init__(self, *, rows_per_cta: int = 16, tile_k: int = 16):
        if (rows_per_cta, tile_k) not in {(16, 16), (8, 8)}:
            raise ValueError(
                "FlashHSASynthetic2DExactGatherScatterTCFwdSm100 requires (rows_per_cta, tile_k) in {(16, 16), (8, 8)}"
            )
        self.rows_per_cta = rows_per_cta
        self.tile_k = tile_k
        self.mma_rows = 16
    ...
    def __call__(
        ...
        mQRowIdx: cute.Tensor,
        mQLength: cute.Tensor,
        mRangeTilePtr: cute.Tensor,
        mTileKRowIdx: cute.Tensor,
        softmax_scale: Float32,
        mOutRows: cute.Tensor,
        mLSERows: cute.Tensor,
        stream: cuda.CUstream,
    ...
    def kernel(
        ...
        tidx, _, _ = cute.arch.thread_idx()
        range_idx, head_idx, _ = cute.arch.block_idx()
        lane = tidx % cute.arch.WARP_SIZE
        smem = cutlass.utils.SmemAllocator()
        mma_rows = self.mma_rows
        logical_rows = self.rows_per_cta
        logical_tile_k = self.tile_k
        tile64_layout = cute.tile_to_shape(
            sm80_utils.get_smem_layout_atom(mQRows.element_type, 64),
            (mma_rows, 64),
            (0, 1),
        ...
        sK = smem.allocate_tensor(
            mKRows.element_type,
            cute.tile_to_shape(
                sm80_utils.get_smem_layout_atom(mKRows.element_type, 64),
                (logical_tile_k, 64),
                (0, 1),
            ),
            byte_alignment=16,
        ...
        sScore = smem.allocate_tensor(
            cutlass.Float32,
            cute.make_layout((mma_rows, logical_tile_k)),
            byte_alignment=16,
        )
        sProb = smem.allocate_tensor(
            cutlass.Float32,
        ...
        sOut = smem.allocate_tensor(
            cutlass.Float32,
            cute.make_layout((mma_rows, 64)),
            byte_alignment=16,
        )
        sRowMax = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows,)), byte_alignment=16)
        sRowSum = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows,)), byte_alignment=16)

        q_length = Int32(mQLength[range_idx])
        tile_begin = Int32(mRangeTilePtr[range_idx])
        tile_end = Int32(mRangeTilePtr[range_idx + Int32(1)])

        for elem_idx in cutlass.range(lane, Int32(mma_rows) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
            row_idx = elem_idx // Int32(64)
            dim_idx = elem_idx - row_idx * Int32(64)
            if row_idx < Int32(logical_rows) and row_idx < q_length:
                global_q_row = Int32(mQRowIdx[range_idx, row_idx])
                if global_q_row >= Int32(0):
                    sQ[row_idx, dim_idx] = mQRows[global_q_row, head_idx, dim_idx]
                else:
                    sQ[row_idx, dim_idx] = Float32(0.0).to(sQ.element_type)
        ...
        tiled_mma = cute.make_tiled_mma(
            warp.MmaF16BF16Op(mQRows.element_type, Float32, (16, 8, 16)),
            (1, 1, 1),
            permutation_mnk=(16, logical_tile_k, 16),
        )
        thr_mma = tiled_mma.get_slice(lane)
        smem_copy_atom = cute.make_copy_atom(
            warp.LdMatrix8x8x16bOp(transpose=False, num_matrices=4),
        ...
        smem_thr_copy_k = utils.make_tiled_copy_B(smem_copy_atom, tiled_mma).get_slice(lane)
        tSrQ = utils.mma_make_fragment_A(sQ, thr_mma)
        tSrK = utils.mma_make_fragment_B(sK, thr_mma)
        tSsQ = smem_thr_copy_q.partition_S(sQ)
        tSsK = smem_thr_copy_k.partition_S(sK)
        acc_shape = thr_mma.partition_shape_C((mma_rows, logical_tile_k))
        c_tile = cute.make_identity_tensor((mma_rows, logical_tile_k))
        tCc = layout_utils.reshape_acc_to_mn(thr_mma.partition_C(c_tile))

        for tile_idx in cutlass.range(tile_begin, tile_end, Int32(1), unroll=1):
            for elem_idx in cutlass.range(lane, Int32(logical_tile_k) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
                tile_row = elem_idx // Int32(64)
                dim_idx = elem_idx - tile_row * Int32(64)
                global_k_row = Int32(mTileKRowIdx[tile_idx, tile_row])
                if global_k_row >= Int32(0):
                    sK[tile_row, dim_idx] = mKRows[global_k_row, head_idx, dim_idx]
                    sV[tile_row, dim_idx] = mVRows[global_k_row, head_idx, dim_idx]
                else:
            ...
            acc_S = cute.make_fragment(acc_shape, Float32)
            acc_S.fill(0.0)
            sm80_utils.gemm(
                thr_mma,
                acc_S,
            ...
            for mi in cutlass.range_constexpr(cute.size(tCc.shape[0])):
                for ni in cutlass.range_constexpr(cute.size(tCc.shape[1])):
                    ...
                    score = -Float32.inf
                    if row_idx < Int32(logical_rows) and row_idx < q_length:
                        score = acc_S_mn[mi, ni] * softmax_scale
                    sScore[row_idx, col_idx] = score
            cute.arch.barrier()
            ...
            if lane < Int32(mma_rows):
                ...
                if active_row:
                    chunk_max = -Float32.inf
                    for tile_col in range(logical_tile_k):
                        score = Float32(sScore[row_idx, tile_col])
                        if score > chunk_max:
                            chunk_max = score
                    if chunk_max != -Float32.inf:
...
class FlashHSASynthetic2DExactTailGatherScatterTCFwdSm100:
    """Benchmark-only fused 8x8 exact core with masked 8x8 tail tiles."""
    ...
    def kernel(
        ...
        sScore = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, logical_tile_k)), byte_alignment=16)
        sProb = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, logical_tile_k)), byte_alignment=16)
        sOut = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, 64)), byte_alignment=16)
        sRowMax = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows,)), byte_alignment=16)
        sRowSum = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows,)), byte_alignment=16)
        ...
        tail_tile_end = Int32(mTailTilePtr[range_idx + Int32(1)])
        for tail_tile_idx in cutlass.range(tail_tile_begin, tail_tile_end, Int32(1), unroll=1):
            for elem_idx in cutlass.range(lane, Int32(logical_tile_k) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
                tile_row = elem_idx // Int32(64)
                dim_idx = elem_idx - tile_row * Int32(64)
                global_k_row = Int32(mTailKRowIdx[tail_tile_idx, tile_row])
                if global_k_row >= Int32(0):
                    sK[tile_row, dim_idx] = mKRows[global_k_row, head_idx, dim_idx]
                    sV[tile_row, dim_idx] = mVRows[global_k_row, head_idx, dim_idx]
                else:
            ...
            for mi in cutlass.range_constexpr(cute.size(tCc.shape[0])):
                for ni in cutlass.range_constexpr(cute.size(tCc.shape[1])):
                    ...
                    col_idx = tCc[mi, ni][1]
                    score = -Float32.inf
                    tail_mask_word = Int32(0)
                    if row_idx < Int32(logical_rows):
                        tail_mask_word = Int32(mTailMaskWords[tail_tile_idx, row_idx, Int32(0)])
                    mask_active = (tail_mask_word & (Int32(1) << col_idx)) != Int32(0)
                    if row_idx < Int32(logical_rows) and row_idx < q_length and mask_active:
                        score = acc_S_mn[mi, ni] * softmax_scale
                    sScore[row_idx, col_idx] = score
            cute.arch.barrier()
...
def _run_cached_zero_three_rows_kernel(
    ...
    compile_key = (
        ...
        dv_rows.shape[1],
        dv_rows.shape[2],
        torch.cuda.get_device_capability(dq_rows.device),
    )
    if compile_key not in _run_cached_zero_three_rows_kernel.compile_cache:
        kernel = FlashHSACachedZeroThreeRowsSm100()
        _run_cached_zero_three_rows_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(row_idx, assumed_align=4, leading_dim=0),
    ...
    _run_cached_zero_three_rows_kernel.compile_cache[compile_key](
        row_idx,
        dq_rows,
        dk_rows,
        dv_rows,
...
def _run_cached_cast_three_rows_kernel(
    ...
    dv_src_rows: torch.Tensor,
    row_idx: torch.Tensor,
    dq_dst_rows: torch.Tensor,
    dk_dst_rows: torch.Tensor,
    dv_dst_rows: torch.Tensor,
) -> None:
    _require_cute_runtime()
    compile_key = (
        "cached_cast_three_rows_v1",
        dq_src_rows.dtype,
        dk_src_rows.dtype,
        dv_src_rows.dtype,
        row_idx.dtype,
        ...
        dv_src_rows.shape[1],
        dv_src_rows.shape[2],
        torch.cuda.get_device_capability(dq_src_rows.device),
    )
    if compile_key not in _run_cached_cast_three_rows_kernel.compile_cache:
        kernel = FlashHSACachedCastThreeRowsSm100()
        _run_cached_cast_three_rows_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(dq_src_rows, assumed_align=4),
            to_cute_tensor(dk_src_rows, assumed_align=4),
            to_cute_tensor(dv_src_rows, assumed_align=4),
            to_cute_tensor(row_idx, assumed_align=4, leading_dim=0),
            to_cute_tensor(dq_dst_rows),
            to_cute_tensor(dk_dst_rows),
            to_cute_tensor(dv_dst_rows),
            cuda.CUstream(torch.cuda.current_stream().cuda_stream),
            options="--enable-tvm-ffi",
        )
    _run_cached_cast_three_rows_kernel.compile_cache[compile_key](
        dq_src_rows,
        dk_src_rows,
        dv_src_rows,
        row_idx,
...
def _run_cached_cast_two_rows_kernel(
    dk_src_rows: torch.Tensor,
    dv_src_rows: torch.Tensor,
    row_idx: torch.Tensor,
    dk_dst_rows: torch.Tensor,
    ...
    compile_key = (
        ...
        dv_src_rows.shape[1],
        dv_src_rows.shape[2],
        torch.cuda.get_device_capability(dk_src_rows.device),
    )
    if compile_key not in _run_cached_cast_two_rows_kernel.compile_cache:
        kernel = FlashHSACachedCastTwoRowsSm100()
        _run_cached_cast_two_rows_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(dk_src_rows, assumed_align=4),
            to_cute_tensor(dv_src_rows, assumed_align=4),
            to_cute_tensor(row_idx, assumed_align=4, leading_dim=0),
    ...
    _run_cached_cast_two_rows_kernel.compile_cache[compile_key](
        dk_src_rows,
        dv_src_rows,
        row_idx,
        dk_dst_rows,
...
_run_cached_cast_two_rows_kernel.compile_cache = get_jit_cache("hsa_cached_cast_two_rows")
...
def _synthetic_sparse_parse_packed_kv_fwd_enabled() -> bool:
    import os

    return os.environ.get("FLASH_ATTN_HSA_SYNTHETIC_PARSE_SPARSE_PACKED_KV_FWD", "0").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
class FlashHSASyntheticDirectMicroBwdDensePackedInputSm100:
    """Direct sparse backward for packed-forward segmented buckets."""
    ...
    def __call__(
        self,
        mQPackedRows: cute.Tensor,
        mKPackedRows: cute.Tensor,
        mVPackedRows: cute.Tensor,
        mOutRows: cute.Tensor,
        mdORows: cute.Tensor,
        mLSERows: cute.Tensor,
        mQRowIdx: cute.Tensor,
    ...
    def kernel(
        ...
        lane = tidx % cute.arch.WARP_SIZE
        smem = cutlass.utils.SmemAllocator()
        sQ = smem.allocate_tensor(mQPackedRows.element_type, cute.make_layout((2, 128)), byte_alignment=16)
        sO = smem.allocate_tensor(mOutRows.element_type, cute.make_layout((2, 128)), byte_alignment=16)
        sdO = smem.allocate_tensor(mdORows.element_type, cute.make_layout((2, 128)), byte_alignment=16)
        sK = smem.allocate_tensor(mKPackedRows.element_type, cute.make_layout((32, 128)), byte_alignment=16)
        sV = smem.allocate_tensor(mVPackedRows.element_type, cute.make_layout((32, 128)), byte_alignment=16)
        q_length = Int32(mQLength[qgroup_idx])
        packed_q_stride = Int32(mQRowIdx.shape[1])
        packed_k_stride = Int32(mKRowIdx.shape[1])
        for elem_idx in cutlass.range(tidx, Int32(2) * Int32(mQPackedRows.shape[2]), self.num_threads, unroll=1):
            row_idx = elem_idx // Int32(mQPackedRows.shape[2])
            dim_idx = elem_idx - row_idx * Int32(mQPackedRows.shape[2])
            if row_idx < q_length:
                global_q_row = Int32(mQRowIdx[qgroup_idx, row_idx])
                if global_q_row >= Int32(0):
                    packed_q_row = qgroup_idx * packed_q_stride + row_idx
                    sQ[row_idx, dim_idx] = mQPackedRows[packed_q_row, head_idx, dim_idx]
                else:
                    sQ[row_idx, dim_idx] = Float32(0.0).to(sQ.element_type)
            else:
                sQ[row_idx, dim_idx] = Float32(0.0).to(sQ.element_type)
        ...
        if warp_idx < Int32(2):
            ...
            for chunk_start in range(0, mKRowIdx.shape[1], cute.arch.WARP_SIZE):
                for elem_idx in cutlass.range(
                    tidx,
                    cute.arch.WARP_SIZE * Int32(mKPackedRows.shape[2]),
                    self.num_threads,
                    unroll=1,
                ):
                    key_rel = elem_idx // Int32(mKPackedRows.shape[2])
                    dim_idx = elem_idx - key_rel * Int32(mKPackedRows.shape[2])
                    key_slot_load = Int32(chunk_start) + key_rel
                    if key_slot_load < k_length:
                        key_row_load = Int32(mKRowIdx[qgroup_idx, key_slot_load])
                        if key_row_load >= Int32(0):
                            packed_key_row = qgroup_idx * packed_k_stride + key_slot_load
                            sK[key_rel, dim_idx] = mKPackedRows[packed_key_row, head_idx, dim_idx]
                            sV[key_rel, dim_idx] = mVPackedRows[packed_key_row, head_idx, dim_idx]
                        else:
                            sK[key_rel, dim_idx] = Float32(0.0).to(sK.element_type)
                            sV[key_rel, dim_idx] = Float32(0.0).to(sV.element_type)
                    else:
                ...
                if active_row:
                    ...
                    if key_slot < k_length:
                        ...
                        if key_row >= Int32(0):
                            score = Float32(0.0)
                            if mQPackedRows.shape[2] in (64, 128):
                                for dim_idx in range(0, mQPackedRows.shape[2], 4):
                                    score += Float32(sQ[warp_idx, dim_idx + 0]) * Float32(sK[lane, dim_idx + 0])
                                    score += Float32(sQ[warp_idx, dim_idx + 1]) * Float32(sK[lane, dim_idx + 1])
                                    score += Float32(sQ[warp_idx, dim_idx + 2]) * Float32(sK[lane, dim_idx + 2])
                                    score += Float32(sQ[warp_idx, dim_idx + 3]) * Float32(sK[lane, dim_idx + 3])
                            else:
                                for dim_idx in range(mQPackedRows.shape[2]):
                                    score += Float32(sQ[warp_idx, dim_idx]) * Float32(sK[lane, dim_idx])
                            prob = cute.math.exp2(score * scale_log2 - lse_log2, fastmath=True)
                            dprob = Float32(0.0)
                            if mdORows.shape[2] in (64, 128):
                                ...
                                for dim_idx in range(mdORows.shape[2]):
                                    dprob += Float32(sdO[warp_idx, dim_idx]) * Float32(sV[lane, dim_idx])
                            ds_scaled = prob * (dprob - dpsum) * softmax_scale
                            for dim_idx in range(lane, mQPackedRows.shape[2], cute.arch.WARP_SIZE):
                                utils.atomic_add_fp32(
                                    ds_scaled * Float32(sQ[warp_idx, dim_idx]),
                                    utils.elem_pointer(mdKRows, (key_row, head_idx, dim_idx)),
                                )
                            for dim_idx in range(lane, mVPackedRows.shape[2], cute.arch.WARP_SIZE):
                                utils.atomic_add_fp32(
                                    prob * Float32(sdO[warp_idx, dim_idx]),
                                    utils.elem_pointer(mdVRows, (key_row, head_idx, dim_idx)),
                                )
                ...
                if active_row:
                    for k_rel in range(cute.arch.WARP_SIZE):
                        ds_k = utils.shuffle_sync(ds_scaled, k_rel)
                        if dim0 < mQPackedRows.shape[2]:
                            dq0 += ds_k * Float32(sK[k_rel, dim0])
                        if dim1 < mQPackedRows.shape[2]:
                            dq1 += ds_k * Float32(sK[k_rel, dim1])
                        if dim2 < mQPackedRows.shape[2]:
                            dq2 += ds_k * Float32(sK[k_rel, dim2])
                        if dim3 < mQPackedRows.shape[2]:
                            dq3 += ds_k * Float32(sK[k_rel, dim3])
                cute.arch.barrier()
            if active_row:
                dim0 = lane
                ...
                dim3 = dim2 + cute.arch.WARP_SIZE
                if dim0 < mdQRows.shape[2]:
                    utils.atomic_add_fp32(dq0, utils.elem_pointer(mdQRows, (global_q_row, head_idx, dim0)))
                if dim1 < mdQRows.shape[2]:
                    utils.atomic_add_fp32(dq1, utils.elem_pointer(mdQRows, (global_q_row, head_idx, dim1)))
                if dim2 < mdQRows.shape[2]:
                    utils.atomic_add_fp32(dq2, utils.elem_pointer(mdQRows, (global_q_row, head_idx, dim2)))
                if dim3 < mdQRows.shape[2]:
                    utils.atomic_add_fp32(dq3, utils.elem_pointer(mdQRows, (global_q_row, head_idx, dim3)))


class FlashHSASyntheticDirectMicroBwdMaskedPackedInputSm100:
    """Masked direct sparse backward for packed-forward segmented buckets."""
...
def _run_synthetic_direct_micro_bwd_dense_packed_input_kernel(
    q_packed_rows: torch.Tensor,
    k_packed_rows: torch.Tensor,
    v_packed_rows: torch.Tensor,
    out_rows: torch.Tensor,
    dout_rows: torch.Tensor,
    lse_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    ...
    *,
    ...
    _require_cute_runtime()
    compile_key = (
        "synthetic_direct_micro_bwd_dense_packed_input_v1",
        q_packed_rows.dtype,
        k_packed_rows.dtype,
        v_packed_rows.dtype,
        out_rows.dtype,
        ...
        q_row_idx.shape[1],
        k_row_idx.shape[1],
        q_packed_rows.shape[1],
        q_packed_rows.shape[2],
        v_packed_rows.shape[2],
        torch.cuda.get_device_capability(q_packed_rows.device),
    )
    if compile_key not in _run_synthetic_direct_micro_bwd_dense_packed_input_kernel.compile_cache:
        kernel = FlashHSASyntheticDirectMicroBwdDensePackedInputSm100()
        _run_synthetic_direct_micro_bwd_dense_packed_input_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_packed_rows),
            to_cute_tensor(k_packed_rows),
            to_cute_tensor(v_packed_rows),
            to_cute_tensor(out_rows),
            to_cute_tensor(dout_rows),
            to_cute_tensor(lse_rows, assumed_align=4),
            to_cute_tensor(q_row_idx, assumed_align=4),
    ...
    _run_synthetic_direct_micro_bwd_dense_packed_input_kernel.compile_cache[compile_key](
        q_packed_rows,
        k_packed_rows,
        v_packed_rows,
        out_rows,
...
_run_synthetic_direct_micro_bwd_dense_packed_input_kernel.compile_cache = get_jit_cache(
    "hsa_synth_direct_micro_bwd_dense_packed_input"
)


def _run_synthetic_direct_micro_bwd_masked_packed_input_kernel(
    q_packed_rows: torch.Tensor,
    k_packed_rows: torch.Tensor,
    v_packed_rows: torch.Tensor,
    out_rows: torch.Tensor,
    ...
    *,
    ...
    _require_cute_runtime()
    compile_key = (
        "synthetic_direct_micro_bwd_masked_packed_input_v1",
        q_packed_rows.dtype,
        k_packed_rows.dtype,
        v_packed_rows.dtype,
        out_rows.dtype,
    ...
    if compile_key not in _run_synthetic_direct_micro_bwd_masked_packed_input_kernel.compile_cache:
        kernel = FlashHSASyntheticDirectMicroBwdMaskedPackedInputSm100()
        _run_synthetic_direct_micro_bwd_masked_packed_input_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_packed_rows),
            to_cute_tensor(k_packed_rows),
            to_cute_tensor(v_packed_rows),
    ...
    _run_synthetic_direct_micro_bwd_masked_packed_input_kernel.compile_cache[compile_key](
        q_packed_rows,
        k_packed_rows,
        v_packed_rows,
        out_rows,
...
_run_synthetic_direct_micro_bwd_masked_packed_input_kernel.compile_cache = get_jit_cache(
    "hsa_synth_direct_micro_bwd_masked_packed_input"
)
...
@cute.jit
def _bucket_dense_ds_from_saved_prob_loaded_v_packed(
    lane16: Int32,
    participates: Boolean,
    do0: Float32,
    do1: Float32,
...
class FlashHSASyntheticDirectRowMicroBwdBucketDenseSavedProbSm100:
    """Qgroup-owned bucket-dense backward that reuses forward-saved compressed probabilities."""
        ...
        for union_slot in range(mUnionKRowIdx.shape[1]):
            ...
            if active_row and slot_valid:
                ...
                if participates:
                    if cutlass.const_expr(mKRows.element_type is cutlass.BFloat16):
                        ...
                        vval3 = Float32(mVRows[union_key_row, head_idx, dim3])
                    if lane16 == Int32(0):
                        saved_prob = Float32(mSavedProbRows[qgroup_idx, row_idx, head_idx, union_slot_i])
            prob = Float32(0.0)
            ds_scaled = Float32(0.0)
            if slot_valid:
                if cutlass.const_expr(mQRows.element_type is cutlass.BFloat16):
                    prob, ds_scaled = _bucket_dense_ds_from_saved_prob_loaded_v_packed(
                        lane16,
                        participates,
                        do0,
                        do1,
            ...
            if participates:
                if cutlass.const_expr(mQRows.element_type is cutlass.BFloat16):
                    ...
                    dv3 = prob * do3
            if slot_valid:
                partner_dk0 = utils.shuffle_sync(dk0, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                partner_dk1 = utils.shuffle_sync(dk1, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                partner_dk2 = utils.shuffle_sync(dk2, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                partner_dk3 = utils.shuffle_sync(dk3, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                partner_dv0 = utils.shuffle_sync(dv0, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                partner_dv1 = utils.shuffle_sync(dv1, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                partner_dv2 = utils.shuffle_sync(dv2, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                partner_dv3 = utils.shuffle_sync(dv3, lane16 + (Int32(1) - row_idx) * Int32(16), width=32)
                if lane < Int32(16):
                    if cutlass.const_expr(mQRows.element_type is cutlass.BFloat16):
                        dk0, dk1 = cute.arch.add_packed_f32x2((dk0, dk1), (partner_dk0, partner_dk1))
                        dk2, dk3 = cute.arch.add_packed_f32x2((dk2, dk3), (partner_dk2, partner_dk3))
...
class FlashHSASyntheticDirectRowMicroBwdBucketDenseSavedPackedSm100:
    """Bucket-dense backward that reuses row-compact packed union K/V and saved probabilities from forward."""
    ...
    def __call__(
        self,
        mQRows: cute.Tensor,
        mPackedKRows: cute.Tensor,
        mPackedVRows: cute.Tensor,
        mOutRows: cute.Tensor,
        mdORows: cute.Tensor,
        mLSERows: cute.Tensor,
        mQRowIdx: cute.Tensor,
    ...
    def kernel(
        ...
        for union_slot in range(mPackedKRows.shape[1]):
            union_slot_i = Int32(union_slot)
            union_key_row = Int32(-1)
            slot_valid = Boolean(False)
            if qgroup_valid and union_slot_i < union_k_length:
            ...
            if active_row and slot_valid:
                ...
                participates = row_slot >= Int32(0)
                if participates:
                    if cutlass.const_expr(mPackedKRows.element_type is cutlass.BFloat16):
                        kval0, kval1, kval2, kval3 = _load_bf16x4_as_f32(
                            utils.elem_pointer(mPackedKRows, (qgroup_idx, union_slot_i, head_idx, dim0)),
                        )
                        vval0, vval1, vval2, vval3 = _load_bf16x4_as_f32(
                            utils.elem_pointer(mPackedVRows, (qgroup_idx, union_slot_i, head_idx, dim0)),
                        )
                    else:
                        kval0 = Float32(mPackedKRows[qgroup_idx, union_slot_i, head_idx, dim0])
                        kval1 = Float32(mPackedKRows[qgroup_idx, union_slot_i, head_idx, dim1])
                        kval2 = Float32(mPackedKRows[qgroup_idx, union_slot_i, head_idx, dim2])
                        kval3 = Float32(mPackedKRows[qgroup_idx, union_slot_i, head_idx, dim3])
                        vval0 = Float32(mPackedVRows[qgroup_idx, union_slot_i, head_idx, dim0])
                        vval1 = Float32(mPackedVRows[qgroup_idx, union_slot_i, head_idx, dim1])
                        vval2 = Float32(mPackedVRows[qgroup_idx, union_slot_i, head_idx, dim2])
                        vval3 = Float32(mPackedVRows[qgroup_idx, union_slot_i, head_idx, dim3])
                    if lane16 == Int32(0):
                        saved_prob = Float32(mSavedProbRows[qgroup_idx, row_idx, head_idx, union_slot_i])
            prob = Float32(0.0)
            ds_scaled = Float32(0.0)
...
class FlashHSASyntheticDirectRowMicroFwdPackedKVSm100:
    """Row-compact direct sparse forward using prepacked union K/V rows."""
    ...
    def kernel(
        ...
        smem = cutlass.utils.SmemAllocator()
        sK = smem.allocate_tensor(
            mPackedKRows.element_type, cute.make_layout((self.qgroups_per_cta, 16, 64)), byte_alignment=16
        )
        sV = smem.allocate_tensor(
            mPackedVRows.element_type, cute.make_layout((self.qgroups_per_cta, 16, 64)), byte_alignment=16
        )
        for elem_idx in cutlass.range(
            tidx, Int32(self.qgroups_per_cta) * Int32(16) * Int32(64), self.num_threads, unroll=1
        ):
            ...
            if qgroup_idx < qgroup_count:
                union_k_length = Int32(mUnionKLength[qgroup_idx])
                if union_slot < union_k_length:
                    sK[qgroup_load, union_slot, dim_idx] = mPackedKRows[qgroup_idx, union_slot, head_idx, dim_idx]
                    sV[qgroup_load, union_slot, dim_idx] = mPackedVRows[qgroup_idx, union_slot, head_idx, dim_idx]
                else:
                    sK[qgroup_load, union_slot, dim_idx] = Float32(0.0).to(sK.element_type)
                    sV[qgroup_load, union_slot, dim_idx] = Float32(0.0).to(sV.element_type)
            else:
        ...
        if subwarp_idx < Int32(self.qgroups_per_cta * 2):
            ...
            out2 = Float32(0.0)
            out3 = Float32(0.0)
            for key_slot in range(mRowKToUnionIdx.shape[2]):
                score_partial = Float32(0.0)
                union_slot = Int32(-1)
                key_valid = Boolean(False)
                if active_row and Int32(key_slot) < row_k_length:
...
def _run_synthetic_direct_row_micro_fwd_packed_kv_kernel(
    q_rows: torch.Tensor,
    packed_k_rows: torch.Tensor,
    packed_v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    row_k_to_union_idx: torch.Tensor,
    q_length: torch.Tensor,
    row_k_length: torch.Tensor,
    ...
    *,
    ...
    qgroups_per_cta = _get_synthetic_qgroups_per_cta()
    compile_key = (
        "synthetic_direct_row_micro_fwd_packed_kv_v1",
        q_rows.dtype,
        packed_k_rows.dtype,
        packed_v_rows.dtype,
        out_rows.dtype,
        lse_rows.dtype,
        q_row_idx.shape[1],
        packed_k_rows.shape[1],
        q_rows.shape[1],
        q_rows.shape[2],
        qgroups_per_cta,
        torch.cuda.get_device_capability(q_rows.device),
    )
    if compile_key not in _run_synthetic_direct_row_micro_fwd_packed_kv_kernel.compile_cache:
        kernel = FlashHSASyntheticDirectRowMicroFwdPackedKVSm100(qgroups_per_cta=qgroups_per_cta)
        _run_synthetic_direct_row_micro_fwd_packed_kv_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(packed_k_rows),
            to_cute_tensor(packed_v_rows),
            to_cute_tensor(q_row_idx, assumed_align=4),
            to_cute_tensor(row_k_to_union_idx, assumed_align=4),
            to_cute_tensor(q_length, assumed_align=4, leading_dim=0),
            to_cute_tensor(row_k_length, assumed_align=4),
    ...
    _run_synthetic_direct_row_micro_fwd_packed_kv_kernel.compile_cache[compile_key](
        q_rows,
        packed_k_rows,
        packed_v_rows,
        q_row_idx,
...
_run_synthetic_direct_row_micro_fwd_packed_kv_kernel.compile_cache = get_jit_cache(
    "hsa_synth_direct_row_micro_fwd_packed_kv"
)


class FlashHSASyntheticDirectRowMicroFwdSaveProbSm100:
    """Row-compact direct sparse forward that also emits final compressed probabilities."""
    ...
    def kernel(
        ...
        sScore = smem.allocate_tensor(Float32, cute.make_layout((self.qgroups_per_cta, 2, 16)), byte_alignment=16)
        for elem_idx in cutlass.range(
            tidx, Int32(self.qgroups_per_cta) * Int32(16) * Int32(64), self.num_threads, unroll=1
        ):
            qgroup_load = elem_idx // (Int32(16) * Int32(64))
        ...
        if subwarp_idx < Int32(self.qgroups_per_cta * 2):
            ...
            if lane16 == Int32(0):
                for union_slot in range(16):
                    sScore[qgroup_in_cta, row_idx, union_slot] = -Float32.inf
            if active_row:
                if dim0 < mQRows.shape[2]:
                    q0 = Float32(mQRows[global_q_row, head_idx, dim0])
                if dim1 < mQRows.shape[2]:
            ...
            for key_slot in range(mRowKToUnionIdx.shape[2]):
                ...
                if lane16 == Int32(0):
                    if key_valid:
                        sScore[qgroup_in_cta, row_idx, union_slot] = score
                        score_scaled = score * softmax_scale
                        next_max = score_scaled if row_max == -Float32.inf or score_scaled > row_max else row_max
                        prev_scale = (
                            Float32(0.0)
            ...
            if active_row:
                if row_max == -Float32.inf or row_sum == Float32(0.0):
                    ...
                    inv_row_sum = Float32(1.0) / row_sum
                    row_lse = row_max + ssa_to_scalar(cute.math.log(scalar_to_ssa(row_sum, Float32), fastmath=True))
                    if dim0 < mOutRows.shape[2]:
                        mOutRows[global_q_row, head_idx, dim0] = (out0 * inv_row_sum).to(mOutRows.element_type)
                    if dim1 < mOutRows.shape[2]:
                        mOutRows[global_q_row, head_idx, dim1] = (out1 * inv_row_sum).to(mOutRows.element_type)
                    ...
                    if dim3 < mOutRows.shape[2]:
                        mOutRows[global_q_row, head_idx, dim3] = (out3 * inv_row_sum).to(mOutRows.element_type)
                    if lane16 == Int32(0):
                        mLSERows[global_q_row, head_idx] = row_lse.to(mLSERows.element_type)
            if lane16 == Int32(0) and qgroup_valid:
                prob_zero = Float32(0.0).to(mSavedProbRows.element_type)
                if active_row and row_max != -Float32.inf and row_sum != Float32(0.0):
                    scale_log2 = softmax_scale * Float32(_LOG2_E)
                    row_lse = row_max + ssa_to_scalar(cute.math.log(scalar_to_ssa(row_sum, Float32), fastmath=True))
                    lse_log2 = row_lse * Float32(_LOG2_E)
                    for union_slot in range(mSavedProbRows.shape[3]):
                        score_saved = sScore[qgroup_in_cta, row_idx, union_slot]
                        if score_saved == -Float32.inf:
                            mSavedProbRows[qgroup_idx, row_idx, head_idx, union_slot] = prob_zero
                        else:
                            prob = cute.math.exp2(score_saved * scale_log2 - lse_log2, fastmath=True)
                            mSavedProbRows[qgroup_idx, row_idx, head_idx, union_slot] = Float32(prob).to(
                                mSavedProbRows.element_type
                            )
                else:
                    for union_slot in range(mSavedProbRows.shape[3]):
                        mSavedProbRows[qgroup_idx, row_idx, head_idx, union_slot] = prob_zero
...
def _run_synthetic_direct_row_micro_fwd_save_prob_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    ...
    *,
    ...
    qgroups_per_cta = _get_synthetic_qgroups_per_cta()
    compile_key = (
        "synthetic_direct_row_micro_fwd_save_prob_inline_v2",
        q_rows.dtype,
        k_rows.dtype,
        v_rows.dtype,
        out_rows.dtype,
        lse_rows.dtype,
        saved_prob_rows.dtype,
        q_row_idx.shape[1],
        union_k_row_idx.shape[1],
        q_rows.shape[1],
        q_rows.shape[2],
    ...
    if compile_key not in _run_synthetic_direct_row_micro_fwd_save_prob_kernel.compile_cache:
        kernel = FlashHSASyntheticDirectRowMicroFwdSaveProbSm100(qgroups_per_cta=qgroups_per_cta)
        _run_synthetic_direct_row_micro_fwd_save_prob_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
            ...
            to_cute_tensor(out_rows, assumed_align=4),
            to_cute_tensor(lse_rows, assumed_align=4),
            to_cute_tensor(saved_prob_rows, assumed_align=4),
            cuda.CUstream(torch.cuda.current_stream().cuda_stream),
            options="--enable-tvm-ffi",
        )
    _run_synthetic_direct_row_micro_fwd_save_prob_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        q_row_idx,
...
_run_synthetic_direct_row_micro_fwd_save_prob_kernel.compile_cache = get_jit_cache(
    "hsa_synth_direct_row_micro_fwd_save_prob"
)


class FlashHSASyntheticDirectRowMicroFwdSavePackedKVSm100:
    """Row-compact direct sparse forward that emits packed union K/V rows and compressed probabilities."""
    ...
    def __call__(
        ...
        mOutRows: cute.Tensor,
        mLSERows: cute.Tensor,
        mSavedPackedKRows: cute.Tensor,
        mSavedPackedVRows: cute.Tensor,
        mSavedProbRows: cute.Tensor,
        stream: cuda.CUstream,
    ):
        grid_x = (mQRowIdx.shape[0] + self.qgroups_per_cta - 1) // self.qgroups_per_cta
        ...
        for elem_idx in cutlass.range(
            ...
            if qgroup_idx < qgroup_count:
                ...
                if union_slot < union_k_length:
                    union_key_row = Int32(mUnionKRowIdx[qgroup_idx, union_slot])
                    if union_key_row >= Int32(0):
                        kval = Float32(mKRows[union_key_row, head_idx, dim_idx])
                        vval = Float32(mVRows[union_key_row, head_idx, dim_idx])
            sK[qgroup_load, union_slot, dim_idx] = kval.to(sK.element_type)
            sV[qgroup_load, union_slot, dim_idx] = vval.to(sV.element_type)
            if qgroup_idx < qgroup_count:
                mSavedPackedKRows[qgroup_idx, union_slot, head_idx, dim_idx] = kval.to(mSavedPackedKRows.element_type)
                mSavedPackedVRows[qgroup_idx, union_slot, head_idx, dim_idx] = vval.to(mSavedPackedVRows.element_type)
        cute.arch.barrier()
        if subwarp_idx < Int32(self.qgroups_per_cta * 2):
            qgroup_idx = qgroup_base + qgroup_in_cta
            qgroup_valid = qgroup_idx < qgroup_count
            ...
            if lane == Int32(0) and qgroup_valid:
                prob_zero = Float32(0.0).to(mSavedProbRows.element_type)
                if active_row and row_max != -Float32.inf and row_sum != Float32(0.0):
                    scale_log2 = softmax_scale * Float32(_LOG2_E)
                    row_lse = row_max + ssa_to_scalar(cute.math.log(scalar_to_ssa(row_sum, Float32), fastmath=True))
...
def _run_synthetic_direct_row_micro_fwd_save_packed_kv_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    ...
    out_rows: torch.Tensor,
    lse_rows: torch.Tensor,
    saved_packed_k_rows: torch.Tensor,
    saved_packed_v_rows: torch.Tensor,
    saved_prob_rows: torch.Tensor,
    *,
    softmax_scale: float,
):
    ...
    qgroups_per_cta = _get_synthetic_qgroups_per_cta()
    compile_key = (
        "synthetic_direct_row_micro_fwd_save_packed_kv_prob_inline_v1",
        q_rows.dtype,
        k_rows.dtype,
        v_rows.dtype,
        out_rows.dtype,
        lse_rows.dtype,
        saved_packed_k_rows.dtype,
        saved_packed_v_rows.dtype,
        saved_prob_rows.dtype,
        q_row_idx.shape[1],
        union_k_row_idx.shape[1],
        q_rows.shape[1],
    ...
    if compile_key not in _run_synthetic_direct_row_micro_fwd_save_packed_kv_kernel.compile_cache:
        kernel = FlashHSASyntheticDirectRowMicroFwdSavePackedKVSm100(qgroups_per_cta=qgroups_per_cta)
        _run_synthetic_direct_row_micro_fwd_save_packed_kv_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
            ...
            to_cute_tensor(out_rows, assumed_align=4),
            to_cute_tensor(lse_rows, assumed_align=4),
            to_cute_tensor(saved_packed_k_rows, assumed_align=4),
            to_cute_tensor(saved_packed_v_rows, assumed_align=4),
            to_cute_tensor(saved_prob_rows, assumed_align=4),
            cuda.CUstream(torch.cuda.current_stream().cuda_stream),
            options="--enable-tvm-ffi",
        )
    _run_synthetic_direct_row_micro_fwd_save_packed_kv_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        q_row_idx,
...
_run_synthetic_direct_row_micro_fwd_save_packed_kv_kernel.compile_cache = get_jit_cache(
    "hsa_synth_direct_row_micro_fwd_save_packed_kv_prob"
)
...
def _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_prob(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    out_rows: torch.Tensor,
    ...
    *,
    ...
    _require_cute_runtime()
    del row_k_row_idx, row_k_to_union_idx
    qgroups_per_cta = _get_synthetic_qgroups_per_cta_bwd()
    if qgroups_per_cta not in (1, 2):
        qgroups_per_cta = 1
    compile_key = (
        "synthetic_direct_row_micro_bwd_bucket_dense_saved_prob_v1",
        q_rows.dtype,
        k_rows.dtype,
        v_rows.dtype,
        out_rows.dtype,
    ...
    if compile_key not in _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_prob.compile_cache:
        kernel = FlashHSASyntheticDirectRowMicroBwdBucketDenseSavedProbSm100(qgroups_per_cta=qgroups_per_cta)
        _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_prob.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
    ...
    _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_prob.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        out_rows,
...
_run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_prob.compile_cache = get_jit_cache(
    "hsa_synth_direct_row_micro_bwd_bucket_dense_saved_prob"
)


def _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_packed(
    q_rows: torch.Tensor,
    packed_k_rows: torch.Tensor,
    packed_v_rows: torch.Tensor,
    out_rows: torch.Tensor,
    ...
    *,
    ...
    del row_k_row_idx, row_k_to_union_idx, row_k_length
    _require_cute_runtime()
    qgroups_per_cta = _get_synthetic_qgroups_per_cta_bwd()
    compile_key = (
        "synthetic_direct_row_micro_bwd_bucket_dense_saved_packed_v2",
        q_rows.dtype,
        packed_k_rows.dtype,
        packed_v_rows.dtype,
        out_rows.dtype,
    ...
    if compile_key not in _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_packed.compile_cache:
        kernel = FlashHSASyntheticDirectRowMicroBwdBucketDenseSavedPackedSm100(qgroups_per_cta=qgroups_per_cta)
        _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_packed.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(packed_k_rows),
            to_cute_tensor(packed_v_rows),
    ...
    _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_packed.compile_cache[compile_key](
        q_rows,
        packed_k_rows,
        packed_v_rows,
        out_rows,
...
_run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_packed.compile_cache = get_jit_cache(
    "hsa_synth_direct_row_micro_bwd_bucket_dense_saved_packed"
)
...
def _can_use_synthetic_bucket_dense_saved_prob_bwd(
    q_rows: torch.Tensor,
    union_k_row_idx: torch.Tensor,
    q_row_idx: torch.Tensor,
) -> bool:
    try:
        ...
        return (
            _can_use_synthetic_bucket_dense_bwd(q_rows, union_k_row_idx, q_row_idx)
            and q_rows.dtype == torch.bfloat16
            and q_rows.shape[0] >= 65536
            and qgroups_per_cta_bwd in (1, 2)
        )
    except Exception:
...
def _can_use_synthetic_bucket_dense_saved_packed_bwd(
    q_rows: torch.Tensor,
    union_k_row_idx: torch.Tensor,
    q_row_idx: torch.Tensor,
) -> bool:
...
def _compute_saved_packed_qkv_offsets(
    direct_bucket_size,
    direct_bucket_packed_q,
    direct_bucket_packed_k,
) -> tuple[list[int], list[int]]:
    q_offsets = [0]
    kv_offsets = [0]
    for bucket_idx in range(len(direct_bucket_size)):
        q_offsets.append(
            q_offsets[-1] + int(direct_bucket_size[bucket_idx]) * int(direct_bucket_packed_q[bucket_idx])
        )
        kv_offsets.append(
            kv_offsets[-1] + int(direct_bucket_size[bucket_idx]) * int(direct_bucket_packed_k[bucket_idx])
        )
    return q_offsets, kv_offsets


def _should_save_synthetic_bucket_dense_prob(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
) -> bool:
    variant = _get_synthetic_one_kernel_bwd_variant()
    if variant != "bucket_dense_saved_prob":
        return False
    try:
        return (
            q_rows.dtype == torch.bfloat16
            and k_rows.dtype == torch.bfloat16
            and v_rows.dtype == torch.bfloat16
            and q_rows.shape[0] >= 65536
            and q_rows.shape[1] == k_rows.shape[1]
            and q_rows.shape[1] == v_rows.shape[1]
            and q_rows.shape[2] == 64
            and k_rows.shape[2] == 64
            and v_rows.shape[2] == 64
        )
    except Exception:
        return False


def _should_save_synthetic_bucket_dense_packed_qkv(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
) -> bool:
    variant = _get_synthetic_one_kernel_bwd_variant()
    if variant != "bucket_dense_saved_packed":
        return False
    try:
        return (
            q_rows.dtype == torch.bfloat16
...
def _get_bucket_dense_saved_packed_qkv_forward_buffers(
    runtime,
    *,
    device: torch.device,
    direct_bucket_size,
    ...
    workspace.setdefault("qgroup_q", {})
    workspace.setdefault("bucket_q", {})
    workspace.setdefault("bucket_kv", {})
    packed_workspace = workspace.setdefault("bucket_dense_saved_packed_qkv", {})
    q_offsets, kv_offsets = _compute_saved_packed_qkv_offsets(
        direct_bucket_size, direct_bucket_packed_q, direct_bucket_packed_k
    )
    q_shape = (q_offsets[-1], num_q_heads, head_dim_q)
    k_shape = (kv_offsets[-1], num_k_heads, head_dim_k)
    v_shape = (kv_offsets[-1], num_v_heads, head_dim_v)
    packed_q = packed_workspace.get("packed_q")
    packed_k = packed_workspace.get("packed_k")
    packed_v = packed_workspace.get("packed_v")
    if (
        packed_q is None
        or tuple(packed_q.shape) != q_shape
        or packed_q.dtype != torch.bfloat16
        or str(packed_q.device) != str(device)
    ):
        packed_q = torch.empty(q_shape, dtype=torch.bfloat16, device=device)
        packed_workspace["packed_q"] = packed_q
    if (
        packed_k is None
        or tuple(packed_k.shape) != k_shape
        or packed_k.dtype != torch.bfloat16
        or str(packed_k.device) != str(device)
    ):
        packed_k = torch.empty(k_shape, dtype=torch.bfloat16, device=device)
        packed_workspace["packed_k"] = packed_k
    if (
        packed_v is None
        or tuple(packed_v.shape) != v_shape
        or packed_v.dtype != torch.bfloat16
        or str(packed_v.device) != str(device)
    ):
        packed_v = torch.empty(v_shape, dtype=torch.bfloat16, device=device)
        packed_workspace["packed_v"] = packed_v
    packed_workspace["token"] = int(runtime.synthetic_forward_prob_token)
    return packed_q, packed_k, packed_v, q_offsets, kv_offsets


def _get_bucket_dense_saved_packed_qkv_cache(
    runtime,
    *,
    device: torch.device,
    direct_bucket_size,
    ...
    head_dim_v: int,
    token: int,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, list[int], list[int]] | None:
    if runtime is None or token <= 0:
        return None
    workspace = runtime.synthetic_forward_workspace
    if workspace is None:
        return None
    packed_workspace = workspace.get("bucket_dense_saved_packed_qkv")
    if not isinstance(packed_workspace, dict):
        return None
    if int(packed_workspace.get("token", -1)) != int(token):
        return None
    packed_q = packed_workspace.get("packed_q")
    packed_k = packed_workspace.get("packed_k")
    packed_v = packed_workspace.get("packed_v")
...
def _get_bucket_dense_saved_packed_union_kv_forward_buffers(
    runtime,
    *,
    device: torch.device,
    num_direct_qgroups: int,
    max_packed_k: int,
    num_k_heads: int,
    head_dim_k: int,
    num_v_heads: int,
    ...
    workspace.setdefault("bucket_q", {})
    workspace.setdefault("bucket_kv", {})
    packed_workspace = workspace.setdefault("bucket_dense_saved_packed_union_kv", {})
    k_shape = (num_direct_qgroups, max_packed_k, num_k_heads, head_dim_k)
    v_shape = (num_direct_qgroups, max_packed_k, num_v_heads, head_dim_v)
    packed_k = packed_workspace.get("packed_k")
    packed_v = packed_workspace.get("packed_v")
    if (
        packed_k is None
...
def _get_bucket_dense_saved_packed_union_kv_cache(
    runtime,
    *,
    device: torch.device,
    num_direct_qgroups: int,
    ...
    if workspace is None:
        return None
    packed_workspace = workspace.get("bucket_dense_saved_packed_union_kv")
    if not isinstance(packed_workspace, dict):
        return None
    if int(packed_workspace.get("token", -1)) != int(token):
        return None
...
def _get_bucket_dense_saved_prob_forward_buffer(
    runtime,
    *,
    device: torch.device,
    num_direct_qgroups: int,
    ...
    workspace.setdefault("bucket_q", {})
    workspace.setdefault("bucket_kv", {})
    saved_prob_workspace = workspace.setdefault("bucket_dense_saved_prob", {})
    shape = (num_direct_qgroups, 2, num_heads, 16)
    prob = saved_prob_workspace.get("prob")
    if (
        prob is None
        or tuple(prob.shape) != shape
        or prob.dtype != dtype
        or str(prob.device) != str(device)
    ):
        prob = torch.empty(shape, dtype=dtype, device=device)
        saved_prob_workspace["prob"] = prob
    saved_prob_workspace["token"] = int(runtime.synthetic_forward_prob_token)
    return prob


def _get_bucket_dense_saved_prob_cache(
    runtime,
    *,
    device: torch.device,
    num_direct_qgroups: int,
    ...
    if workspace is None:
        return None
    saved_prob_workspace = workspace.get("bucket_dense_saved_prob")
    if not isinstance(saved_prob_workspace, dict):
        return None
    if int(saved_prob_workspace.get("token", -1)) != int(token):
        return None
    prob = saved_prob_workspace.get("prob")
    expected_shape = (num_direct_qgroups, 2, num_heads, 16)
    if (
        prob is None
        or tuple(prob.shape) != expected_shape
        or prob.dtype != dtype
        or str(prob.device) != str(device)
    ):
        return None
...
def _run_synthetic_direct_row_micro_bwd_kernel_row_compact_one_kernel(
    ...
    dk_rows: torch.Tensor,
    dv_rows: torch.Tensor,
    saved_prob_rows: torch.Tensor | None = None,
    saved_packed_k_rows: torch.Tensor | None = None,
    saved_packed_v_rows: torch.Tensor | None = None,
    *,
    softmax_scale: float,
    max_unique_key_occurrences: int,
    workspace: dict,
    ...
    if variant == "bucket_dense_saved_packed":
        if (
            saved_prob_rows is not None
            and saved_packed_k_rows is not None
            and saved_packed_v_rows is not None
            and _can_use_synthetic_bucket_dense_saved_packed_bwd(q_rows, union_k_row_idx, q_row_idx)
        ):
            return _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_packed(
                q_rows,
                saved_packed_k_rows,
                saved_packed_v_rows,
                out_rows,
    ...
    if variant == "bucket_dense_saved_prob":
        if (
            saved_prob_rows is not None
            and _can_use_synthetic_bucket_dense_saved_prob_bwd(q_rows, union_k_row_idx, q_row_idx)
        ):
            return _run_synthetic_direct_row_micro_bwd_kernel_bucket_dense_saved_prob(
                q_rows,
                k_rows,
                v_rows,
                out_rows,
...
def _run_shared_support_bucket_fwd_kernel(
    ...
    *,
    softmax_scale: float,
    qgroups_per_cta: int = 4,
) -> tuple[torch.Tensor, torch.Tensor]:
    _require_cute_runtime()
    use_wide64 = (
        q_packed.shape[1] == 2
        and q_packed.shape[3] == 64
        and k_shared.shape[2] == 64
        and v_shared.shape[2] == 64
        and int(qgroups_per_cta) in (1, 2, 4)
    )
    out = torch.empty(
        (q_packed.shape[0], q_packed.shape[1], q_packed.shape[2], v_shared.shape[2]),
        dtype=torch.float32,
        device=q_packed.device,
    )
    lse = torch.empty((q_packed.shape[0], q_packed.shape[1], q_packed.shape[2]), dtype=torch.float32, device=q_packed.device)
    compile_key = (
        "shared_support_bucket_fwd_v2_wide64" if use_wide64 else "shared_support_bucket_fwd_v1",
        q_packed.dtype,
        k_shared.dtype,
        v_shared.dtype,
        q_packed.shape[0],
    ...
    if compile_key not in _run_shared_support_bucket_fwd_kernel.compile_cache:
        if use_wide64:
            kernel = FlashHSASharedSupportBucketFwdWide64Sm100(qgroups_per_cta=qgroups_per_cta)
        else:
            kernel = FlashHSASharedSupportBucketFwdSm100(qgroups_per_cta=qgroups_per_cta)
        _run_shared_support_bucket_fwd_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_packed),
            to_cute_tensor(k_shared),
            to_cute_tensor(v_shared),
            to_cute_tensor(q_length, assumed_align=4, leading_dim=0),
            to_cute_tensor(mask_words, assumed_align=4, leading_dim=2),
            Float32(softmax_scale),
            to_cute_tensor(out, assumed_align=4),
    ...
    _run_shared_support_bucket_fwd_kernel.compile_cache[compile_key](
        q_packed,
        k_shared,
        v_shared,
        q_length,
...
def _can_use_synthetic_2d_masked_fwd(
    ...
    *,
    ...
    try:
        max_packed_k = int(os.environ.get("FLASH_ATTN_HSA_CACHED_GATHER_MAX_PACKED_K", "2048"))
    except ValueError:
        max_packed_k = 128
    max_packed_k = max(1, min(max_packed_k, 2048))
    head_dim = int(q_rows.shape[-1])
    if head_dim == 128:
    ...
    return (
        ...
        and int(k_rows.shape[-1]) == head_dim
        and int(v_rows.shape[-1]) == head_dim
        and q_rows.is_cuda
        and k_rows.is_cuda
        and v_rows.is_cuda
    )
...
def _run_synthetic_2d_masked_fwd_kernel(
    ...
    *,
    ...
    head_dim = int(q_packed.shape[3])
    if head_dim == 128:
        kernel_name = "synthetic_2d_masked_fwd_d128_v1"
        kernel_cls = FlashHSASynthetic2DMaskedFwdD128Sm100
    elif head_dim == 64:
        kernel_name = "synthetic_2d_masked_fwd_v1"
        kernel_cls = FlashHSASynthetic2DMaskedFwdSm100
    else:
        raise RuntimeError("synthetic_2d_masked_fwd_unsupported_head_dim")
    compile_key = (
        kernel_name,
        q_packed.dtype,
        k_packed.dtype,
...
def _run_synthetic_2d_masked_gather_fwd_kernel(
    ...
    *,
    ...
    if head_dim == 128:
        kernel_name = "synthetic_2d_masked_gather_fwd_d128_v1"
        kernel_cls = FlashHSASynthetic2DMaskedGatherFwdD128Sm100
    elif head_dim == 64:
        kernel_name = "synthetic_2d_masked_gather_fwd_v1"
        kernel_cls = FlashHSASynthetic2DMaskedGatherFwdSm100
    else:
        raise RuntimeError("synthetic_2d_masked_gather_fwd_unsupported_head_dim")
    compile_key = (
        kernel_name,
        q_rows.dtype,
        k_rows.dtype,
...
def _run_synthetic_2d_masked_gather_scatter_fwd_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    ...
    *,
    ...
    head_dim = int(q_rows.shape[2])
    if head_dim == 128:
        kernel_name = "synthetic_2d_masked_gather_scatter_fwd_d128_v1"
        kernel_cls = FlashHSASynthetic2DMaskedGatherScatterFwdD128Sm100
    elif head_dim == 64:
        kernel_name = "synthetic_2d_masked_gather_scatter_fwd_v1"
        kernel_cls = FlashHSASynthetic2DMaskedGatherScatterFwdSm100
    else:
        raise RuntimeError("synthetic_2d_masked_gather_scatter_fwd_unsupported_head_dim")
    compile_key = (
        kernel_name,
        q_rows.dtype,
        k_rows.dtype,
    ...
    if compile_key not in _run_synthetic_2d_masked_gather_scatter_fwd_kernel.compile_cache:
        kernel = kernel_cls(rows_per_cta=rows_per_cta, tile_k=tile_k)
        _run_synthetic_2d_masked_gather_scatter_fwd_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
    ...
    _run_synthetic_2d_masked_gather_scatter_fwd_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        q_row_idx,
...
_run_synthetic_2d_masked_gather_scatter_fwd_kernel.compile_cache = get_jit_cache(
    "hsa_synth_2d_masked_gather_scatter_fwd"
)


def _run_synthetic_2d_masked_gather_combine_fwd_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    ...
    *,
    ...
    head_dim = int(q_rows.shape[2])
    if head_dim == 128:
        kernel_name = "synthetic_2d_masked_gather_combine_fwd_d128_v1"
        kernel_cls = FlashHSASynthetic2DMaskedGatherCombineFwdD128Sm100
    elif head_dim == 64:
        kernel_name = "synthetic_2d_masked_gather_combine_fwd_v1"
        kernel_cls = FlashHSASynthetic2DMaskedGatherCombineFwdSm100
    else:
        raise RuntimeError("synthetic_2d_masked_gather_combine_fwd_unsupported_head_dim")
    compile_key = (
        kernel_name,
        q_rows.dtype,
        k_rows.dtype,
    ...
    if compile_key not in _run_synthetic_2d_masked_gather_combine_fwd_kernel.compile_cache:
        kernel = kernel_cls(rows_per_cta=rows_per_cta, tile_k=tile_k)
        _run_synthetic_2d_masked_gather_combine_fwd_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
    ...
    _run_synthetic_2d_masked_gather_combine_fwd_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        q_row_idx,
...
_run_synthetic_2d_masked_gather_combine_fwd_kernel.compile_cache = get_jit_cache(
    "hsa_synth_2d_masked_gather_combine_fwd"
)


def _run_synthetic_2d_masked_gather_scatter_tc_fwd_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    ...
    *,
    ...
    _run_synthetic_2d_masked_gather_scatter_tc_fwd_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        q_row_idx,
...
_run_synthetic_2d_masked_gather_scatter_tc_fwd_kernel.compile_cache = get_jit_cache(
    "hsa_synth_2d_masked_gather_scatter_tc_fwd"
)


def _run_synthetic_2d_exact_gather_scatter_tc_fwd_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    q_length: torch.Tensor,
    range_tile_ptr: torch.Tensor,
    tile_k_row_idx: torch.Tensor,
    out_rows: torch.Tensor,
    lse_rows: torch.Tensor,
    *,
    softmax_scale: float,
    kernel_family: str = "tc16x16",
) -> None:
    _require_cute_runtime()
    rows_per_cta = int(q_row_idx.shape[1])
    tile_k = int(tile_k_row_idx.shape[1])
    if (rows_per_cta, tile_k) not in {(16, 16), (8, 8)}:
        raise RuntimeError("synthetic_2d_exact_gather_scatter_tc_fwd_unsupported_shape")
    if str(kernel_family) not in {"tc16x16", "tc8x8"}:
        raise RuntimeError("synthetic_2d_exact_gather_scatter_tc_fwd_unsupported_family")
    if (rows_per_cta, tile_k) != ((16, 16) if str(kernel_family) == "tc16x16" else (8, 8)):
        raise RuntimeError("synthetic_2d_exact_gather_scatter_tc_fwd_shape_family_mismatch")
    compile_key = (
        "synthetic_2d_exact_gather_scatter_tc_fwd_v2",
        str(kernel_family),
        q_rows.dtype,
        k_rows.dtype,
        v_rows.dtype,
        ...
        rows_per_cta,
        tile_k,
        tile_k_row_idx.shape[0],
        q_rows.shape[1],
        q_rows.shape[2],
        v_rows.shape[2],
        torch.cuda.get_device_capability(q_rows.device),
    )
    if compile_key not in _run_synthetic_2d_exact_gather_scatter_tc_fwd_kernel.compile_cache:
        kernel = FlashHSASynthetic2DExactGatherScatterTCFwdSm100(rows_per_cta=rows_per_cta, tile_k=tile_k)
        _run_synthetic_2d_exact_gather_scatter_tc_fwd_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
            to_cute_tensor(q_row_idx, assumed_align=4),
            to_cute_tensor(q_length, assumed_align=4, leading_dim=0),
            to_cute_tensor(range_tile_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(tile_k_row_idx, assumed_align=4),
            Float32(softmax_scale),
            to_cute_tensor(out_rows, assumed_align=4),
            to_cute_tensor(lse_rows, assumed_align=4),
            cuda.CUstream(torch.cuda.current_stream().cuda_stream),
    ...
    _run_synthetic_2d_exact_gather_scatter_tc_fwd_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        q_row_idx,
...
_run_synthetic_2d_exact_gather_scatter_tc_fwd_kernel.compile_cache = get_jit_cache(
    "hsa_synth_2d_exact_gather_scatter_tc_fwd"
)


def _run_synthetic_2d_exact_tail_gather_scatter_tc_fwd_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    q_row_idx: torch.Tensor,
    ...
    range_tile_ptr: torch.Tensor,
    tile_k_row_idx: torch.Tensor,
    tail_tile_ptr: torch.Tensor,
    tail_k_row_idx: torch.Tensor,
    tail_mask_words: torch.Tensor,
    out_rows: torch.Tensor,
    lse_rows: torch.Tensor,
    *,
    softmax_scale: float,
    ...
    _require_cute_runtime()
    rows_per_cta = int(q_row_idx.shape[1])
    tile_k = int(tile_k_row_idx.shape[1]) if int(tile_k_row_idx.ndim) > 1 and int(tile_k_row_idx.shape[0]) > 0 else 8
    if rows_per_cta != 8 or tile_k != 8:
        raise RuntimeError("synthetic_2d_exact_tail_gather_scatter_tc_fwd_unsupported_shape")
    compile_key = (
        "synthetic_2d_exact_tail_gather_scatter_tc_fwd_v2",
        q_rows.dtype,
        k_rows.dtype,
        v_rows.dtype,
        out_rows.dtype,
        ...
        q_row_idx.shape[0],
        tile_k_row_idx.shape[0],
        tail_tile_ptr.shape[0],
        tail_k_row_idx.shape[0],
        tail_mask_words.shape[0],
        q_rows.shape[1],
        q_rows.shape[2],
        v_rows.shape[2],
        torch.cuda.get_device_capability(q_rows.device),
    )
    if compile_key not in _run_synthetic_2d_exact_tail_gather_scatter_tc_fwd_kernel.compile_cache:
        kernel = FlashHSASynthetic2DExactTailGatherScatterTCFwdSm100()
        _run_synthetic_2d_exact_tail_gather_scatter_tc_fwd_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
            ...
            to_cute_tensor(range_tile_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(tile_k_row_idx, assumed_align=4),
            to_cute_tensor(tail_tile_ptr, assumed_align=4, leading_dim=0),
    ...
    _run_synthetic_2d_exact_tail_gather_scatter_tc_fwd_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        q_row_idx,
...
_run_synthetic_2d_exact_tail_gather_scatter_tc_fwd_kernel.compile_cache = get_jit_cache(
    "hsa_synth_2d_exact_tail_gather_scatter_tc_fwd"
)


@cute.jit
def _load_cached_generalized_fused_occurrence_row_state(
    mQRows: cute.Tensor,
    mOutRows: cute.Tensor,
    mdORows: cute.Tensor,
    mLSERows: cute.Tensor,
    ...
    q_length = Int32(mQLength[range_idx])
    if row_i < q_length:
        global_q_row = Int32(mQRowIdx[range_idx, row_i])
        if global_q_row >= Int32(0):
            if occurrence_kind == Int32(0):
                participates = Boolean(True)
            else:
                tail_mask_word = Int32(mTailMaskWords[tile_idx, row_i, Int32(0)])
                participates = (tail_mask_word & (Int32(1) << col_idx)) != Int32(0)
    if participates:
        if dim0 < mQRows.shape[2]:
            q0 = Float32(mQRows[global_q_row, head_idx, dim0])
            out0 = Float32(mOutRows[global_q_row, head_idx, dim0])
...
class FlashHSACachedGeneralizedFusedBwdDQSm100:
    """Warp-sized tensor-core backward over cached fused 8x8 exact+tail ranges."""

    arch = 100

    def __init__(self):
        self.rows_per_range = 8
        self.keys_per_tile = 8
        self.mma_rows = 16
        self.num_threads = 32
    ...
    def __call__(
        ...
        mQRowIdx: cute.Tensor,
        mQLength: cute.Tensor,
        mExactTilePtr: cute.Tensor,
        mExactKRowIdx: cute.Tensor,
        mTailTilePtr: cute.Tensor,
        mTailKRowIdx: cute.Tensor,
        mTailMaskWords: cute.Tensor,
        softmax_scale: Float32,
    ...
    def kernel(
        ...
        smem = cutlass.utils.SmemAllocator()
        mma_rows = self.mma_rows
        logical_rows = self.rows_per_range
        logical_tile_k = self.keys_per_tile
        q_length = Int32(mQLength[range_idx])
        tile64_layout = cute.tile_to_shape(
            sm80_utils.get_smem_layout_atom(mQRows.element_type, 64),
            (mma_rows, 64),
        ...
        sQ = smem.allocate_tensor(mQRows.element_type, tile64_layout, byte_alignment=16)
        sdO = smem.allocate_tensor(mdORows.element_type, tile64_layout, byte_alignment=16)
        sOutRows = smem.allocate_tensor(mOutRows.element_type, tile64_layout, byte_alignment=16)
        sK = smem.allocate_tensor(
            mKRows.element_type,
            cute.tile_to_shape(
                sm80_utils.get_smem_layout_atom(mKRows.element_type, 64),
        ...
        sScore = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, logical_tile_k)), byte_alignment=16)
        sDProb = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, logical_tile_k)), byte_alignment=16)
        sProb = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, logical_tile_k)), byte_alignment=16)
        sDS = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, logical_tile_k)), byte_alignment=16)
        sDQ = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows, 64)), byte_alignment=16)
        sRowDelta = smem.allocate_tensor(cutlass.Float32, cute.make_layout((mma_rows,)), byte_alignment=16)
        ...
        for elem_idx in cutlass.range(lane, Int32(mma_rows) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
            ...
            if row_idx < Int32(logical_rows) and row_idx < q_length:
                ...
                if global_q_row >= Int32(0):
                    sQ[row_idx, dim_idx] = mQRows[global_q_row, head_idx, dim_idx]
                    sdO[row_idx, dim_idx] = mdORows[global_q_row, head_idx, dim_idx]
                    sOutRows[row_idx, dim_idx] = mOutRows[global_q_row, head_idx, dim_idx]
                else:
                    sQ[row_idx, dim_idx] = Float32(0.0).to(sQ.element_type)
                    sdO[row_idx, dim_idx] = Float32(0.0).to(sdO.element_type)
                    sOutRows[row_idx, dim_idx] = Float32(0.0).to(sOutRows.element_type)
            else:
                sQ[row_idx, dim_idx] = Float32(0.0).to(sQ.element_type)
                sdO[row_idx, dim_idx] = Float32(0.0).to(sdO.element_type)
                sOutRows[row_idx, dim_idx] = Float32(0.0).to(sOutRows.element_type)
            sDQ[row_idx, dim_idx] = Float32(0.0)
        if lane < Int32(mma_rows):
            row_idx = lane
            row_delta = Float32(0.0)
            if row_idx < Int32(logical_rows) and row_idx < q_length:
                global_q_row = Int32(mQRowIdx[range_idx, row_idx])
                if global_q_row >= Int32(0):
                    for dim_idx in range(64):
                        row_delta += Float32(sOutRows[row_idx, dim_idx]) * Float32(sdO[row_idx, dim_idx])
            sRowDelta[row_idx] = row_delta
        cute.arch.barrier()
        ...
        smem_thr_copy_q = utils.make_tiled_copy_A(smem_copy_atom, tiled_mma).get_slice(lane)
        smem_thr_copy_k = utils.make_tiled_copy_B(smem_copy_atom, tiled_mma).get_slice(lane)
        smem_thr_copy_do = utils.make_tiled_copy_A(smem_copy_atom, tiled_mma).get_slice(lane)
        smem_thr_copy_v = utils.make_tiled_copy_B(smem_copy_atom, tiled_mma).get_slice(lane)
        tSrQ = utils.mma_make_fragment_A(sQ, thr_mma)
        tSrK = utils.mma_make_fragment_B(sK, thr_mma)
        tSrDO = utils.mma_make_fragment_A(sdO, thr_mma)
        tSrV = utils.mma_make_fragment_B(sV, thr_mma)
        tSsQ = smem_thr_copy_q.partition_S(sQ)
        tSsK = smem_thr_copy_k.partition_S(sK)
        ...
        exact_tile_begin = Int32(mExactTilePtr[range_idx])
        exact_tile_end = Int32(mExactTilePtr[range_idx + Int32(1)])
        for tile_idx in cutlass.range(exact_tile_begin, exact_tile_end, Int32(1), unroll=1):
            for elem_idx in cutlass.range(lane, Int32(logical_tile_k) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
                tile_row = elem_idx // Int32(64)
                dim_idx = elem_idx - tile_row * Int32(64)
                global_k_row = Int32(mExactKRowIdx[tile_idx, tile_row])
                if global_k_row >= Int32(0):
                    sK[tile_row, dim_idx] = mKRows[global_k_row, head_idx, dim_idx]
                    sV[tile_row, dim_idx] = mVRows[global_k_row, head_idx, dim_idx]
                else:
            ...
            acc_DP = cute.make_fragment(acc_shape, Float32)
            acc_DP.fill(0.0)
            sm80_utils.gemm(
                thr_mma,
                acc_DP,
            ...
            for mi in cutlass.range_constexpr(cute.size(tCc.shape[0])):
                for ni in cutlass.range_constexpr(cute.size(tCc.shape[1])):
                    ...
                    if row_idx < Int32(logical_rows) and row_idx < q_length:
                        global_q_row = Int32(mQRowIdx[range_idx, row_idx])
                        global_k_row = Int32(mExactKRowIdx[tile_idx, col_idx])
                        if global_q_row >= Int32(0) and global_k_row >= Int32(0):
                            score = acc_S_mn[mi, ni] * softmax_scale
                            dprob = acc_DP_mn[mi, ni]
                    sScore[row_idx, col_idx] = score
                    sDProb[row_idx, col_idx] = dprob
            cute.arch.barrier()
            ...
            if lane < Int32(mma_rows):
                ...
                if row_idx < Int32(logical_rows) and row_idx < q_length:
                    ...
                    if global_q_row >= Int32(0):
                        lse_log2 = Float32(mLSERows[global_q_row, head_idx]) * Float32(_LOG2_E)
                        row_delta = Float32(sRowDelta[row_idx])
                        for tile_col in range(logical_tile_k):
                            score = Float32(sScore[row_idx, tile_col])
                            prob = Float32(0.0)
                            ds = Float32(0.0)
                            if score != -Float32.inf:
                                prob = cute.math.exp2(score * Float32(_LOG2_E) - lse_log2, fastmath=True)
                                ds = prob * (Float32(sDProb[row_idx, tile_col]) - row_delta) * softmax_scale
                            sProb[row_idx, tile_col] = prob
                            sDS[row_idx, tile_col] = ds
                    else:
                        for tile_col in range(logical_tile_k):
                            sProb[row_idx, tile_col] = Float32(0.0)
                            sDS[row_idx, tile_col] = Float32(0.0)
                else:
                    for tile_col in range(logical_tile_k):
                        sProb[row_idx, tile_col] = Float32(0.0)
                        sDS[row_idx, tile_col] = Float32(0.0)
            ...
            for elem_idx in cutlass.range(lane, Int32(logical_rows) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
                row_idx = elem_idx // Int32(64)
                dim_idx = elem_idx - row_idx * Int32(64)
                dq_val = Float32(sDQ[row_idx, dim_idx])
                for tile_col in range(logical_tile_k):
                    dq_val += Float32(sDS[row_idx, tile_col]) * Float32(sK[tile_col, dim_idx])
        ...
        tail_tile_begin = Int32(mTailTilePtr[range_idx])
        tail_tile_end = Int32(mTailTilePtr[range_idx + Int32(1)])
        for tile_idx in cutlass.range(tail_tile_begin, tail_tile_end, Int32(1), unroll=1):
            for elem_idx in cutlass.range(lane, Int32(logical_tile_k) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
                tile_row = elem_idx // Int32(64)
                dim_idx = elem_idx - tile_row * Int32(64)
                global_k_row = Int32(mTailKRowIdx[tile_idx, tile_row])
                if global_k_row >= Int32(0):
                    sK[tile_row, dim_idx] = mKRows[global_k_row, head_idx, dim_idx]
                    sV[tile_row, dim_idx] = mVRows[global_k_row, head_idx, dim_idx]
                else:
            ...
            for mi in cutlass.range_constexpr(cute.size(tCc.shape[0])):
                for ni in cutlass.range_constexpr(cute.size(tCc.shape[1])):
                    ...
                    score = -Float32.inf
                    dprob = Float32(0.0)
                    mask_active = Boolean(False)
                    if row_idx < Int32(logical_rows):
                        tail_mask_word = Int32(mTailMaskWords[tile_idx, row_idx, Int32(0)])
                        mask_active = (tail_mask_word & (Int32(1) << col_idx)) != Int32(0)
                    if row_idx < Int32(logical_rows) and row_idx < q_length:
                        global_q_row = Int32(mQRowIdx[range_idx, row_idx])
                        global_k_row = Int32(mTailKRowIdx[tile_idx, col_idx])
                        if global_q_row >= Int32(0) and global_k_row >= Int32(0) and mask_active:
                            score = acc_S_mn[mi, ni] * softmax_scale
                            dprob = acc_DP_mn[mi, ni]
                    sScore[row_idx, col_idx] = score
                    sDProb[row_idx, col_idx] = dprob
        ...
        for elem_idx in cutlass.range(lane, Int32(logical_rows) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
            ...
            if row_idx < q_length:
                global_q_row = Int32(mQRowIdx[range_idx, row_idx])
                if global_q_row >= Int32(0):
                    mdQRows[global_q_row, head_idx, dim_idx] = Float32(sDQ[row_idx, dim_idx]).to(
                        mdQRows.element_type
                    )


class FlashHSACachedGeneralizedFusedBwdKeyOwnedSm100:
    """Key-owned dK/dV backward with shared row-state reuse per occurrence."""
    ...
    def __call__(
        ...
        mQLength: cute.Tensor,
        mTailMaskWords: cute.Tensor,
        mOwnedKRowIdx: cute.Tensor,
        mOwnedOccurrencePtr: cute.Tensor,
        mOwnedOccurrenceKind: cute.Tensor,
        mOwnedOccurrenceRangeIdx: cute.Tensor,
        mOwnedOccurrenceTileIdx: cute.Tensor,
        mOwnedOccurrenceColIdx: cute.Tensor,
        softmax_scale: Float32,
        mdKRows: cute.Tensor,
        mdVRows: cute.Tensor,
        stream: cuda.CUstream,
    ):
        grid_x = mOwnedKRowIdx.shape[0]
        grid_y = mQRows.shape[1]
        self.kernel(
            mQRows,
            mKRows,
            ...
            mOwnedKRowIdx,
            mOwnedOccurrencePtr,
            mOwnedOccurrenceKind,
            mOwnedOccurrenceRangeIdx,
            mOwnedOccurrenceTileIdx,
            mOwnedOccurrenceColIdx,
            softmax_scale,
            mdKRows,
            mdVRows,
        ).launch(
    ...
    def kernel(
        ...
        tidx, _, _ = cute.arch.thread_idx()
        unique_key_idx, head_idx, _ = cute.arch.block_idx()
        lane = tidx
        dim0 = lane * Int32(4)
        dim1 = dim0 + Int32(1)
        dim2 = dim0 + Int32(2)
        dim3 = dim0 + Int32(3)
        key_count = Int32(mOwnedKRowIdx.shape[0])
        active_key = unique_key_idx < key_count
        key_row = Int32(-1)
        occ_start = Int32(0)
        occ_end = Int32(0)
        if active_key:
            key_row = Int32(mOwnedKRowIdx[unique_key_idx])
            occ_start = Int32(mOwnedOccurrencePtr[unique_key_idx])
            occ_end = Int32(mOwnedOccurrencePtr[unique_key_idx + Int32(1)])
        ...
        sQ = smem.allocate_tensor(
            mQRows.element_type,
            cute.make_layout((self.rows_per_range, 64)),
            byte_alignment=16,
        )
        sdO = smem.allocate_tensor(
            mdORows.element_type,
            cute.make_layout((self.rows_per_range, 64)),
            byte_alignment=16,
        )
        sOut = smem.allocate_tensor(
        ...
        sRowDelta = smem.allocate_tensor(cutlass.Float32, cute.make_layout((self.rows_per_range,)), byte_alignment=16)
        sRowLSE = smem.allocate_tensor(cutlass.Float32, cute.make_layout((self.rows_per_range,)), byte_alignment=16)
        sRowValid = smem.allocate_tensor(cutlass.Int32, cute.make_layout((self.rows_per_range,)), byte_alignment=4)
        sRowActive = smem.allocate_tensor(cutlass.Int32, cute.make_layout((self.rows_per_range,)), byte_alignment=4)
        current_range_idx = Int32(-1)
        current_q_length = Int32(0)

        for occ_idx in cutlass.range(occ_start, occ_end, Int32(1), unroll=1):
            occurrence_kind = Int32(mOwnedOccurrenceKind[occ_idx])
            range_idx = Int32(mOwnedOccurrenceRangeIdx[occ_idx])
            tile_idx = Int32(mOwnedOccurrenceTileIdx[occ_idx])
            col_idx = Int32(mOwnedOccurrenceColIdx[occ_idx])
            if range_idx != current_range_idx:
                current_range_idx = range_idx
                current_q_length = Int32(mQLength[range_idx])
                for elem_idx in cutlass.range(
                    tidx, Int32(self.rows_per_range) * Int32(64), Int32(self.num_threads), unroll=1
                ):
                    row_i = elem_idx // Int32(64)
                    dim_idx = elem_idx - row_i * Int32(64)
                    global_q_row = Int32(-1)
                    if row_i < current_q_length:
                        global_q_row = Int32(mQRowIdx[range_idx, row_i])
                    if global_q_row >= Int32(0):
                        sQ[row_i, dim_idx] = mQRows[global_q_row, head_idx, dim_idx]
                        sdO[row_i, dim_idx] = mdORows[global_q_row, head_idx, dim_idx]
                        sOut[row_i, dim_idx] = mOutRows[global_q_row, head_idx, dim_idx]
                    else:
                        sQ[row_i, dim_idx] = Float32(0.0).to(sQ.element_type)
                        sdO[row_i, dim_idx] = Float32(0.0).to(sdO.element_type)
                        sOut[row_i, dim_idx] = Float32(0.0).to(sOut.element_type)
                if lane < Int32(self.rows_per_range):
                    row_i = lane
                    global_q_row = Int32(-1)
                    row_valid = Int32(0)
                    if row_i < current_q_length:
                    ...
                    if global_q_row >= Int32(0):
                        row_valid = Int32(1)
                    row_delta = Float32(0.0)
                    if row_valid != Int32(0):
                        for dim_idx in range(64):
                            row_delta += Float32(sOut[row_i, dim_idx]) * Float32(sdO[row_i, dim_idx])
                    sRowDelta[row_i] = row_delta
                    sRowLSE[row_i] = (
                        Float32(mLSERows[global_q_row, head_idx]) * Float32(_LOG2_E)
                        if row_valid != Int32(0)
                        else Float32(0.0)
                    )
                    sRowValid[row_i] = row_valid
                cute.arch.barrier()
            q_length = current_q_length
            if lane < Int32(self.rows_per_range):
                row_i = lane
                row_active = Int32(sRowValid[row_i])
                if row_active != Int32(0) and occurrence_kind == Int32(1):
                    tail_mask_word = Int32(mTailMaskWords[tile_idx, row_i, Int32(0)])
                    row_active = Int32(1) if ((tail_mask_word & (Int32(1) << col_idx)) != Int32(0)) else Int32(0)
                sRowActive[row_i] = row_active
            cute.arch.barrier()

            for row_idx in range(self.rows_per_range):
                row_i = Int32(row_idx)
                participates = row_i < q_length and sRowActive[row_i] != Int32(0)
                q0 = Float32(sQ[row_i, dim0]) if participates else Float32(0.0)
                q1 = Float32(sQ[row_i, dim1]) if participates else Float32(0.0)
                q2 = Float32(sQ[row_i, dim2]) if participates else Float32(0.0)
                q3 = Float32(sQ[row_i, dim3]) if participates else Float32(0.0)
                do0 = Float32(sdO[row_i, dim0]) if participates else Float32(0.0)
                do1 = Float32(sdO[row_i, dim1]) if participates else Float32(0.0)
                do2 = Float32(sdO[row_i, dim2]) if participates else Float32(0.0)
                do3 = Float32(sdO[row_i, dim3]) if participates else Float32(0.0)
                qk_partial = Float32(0.0)
                dprob_partial = Float32(0.0)
                if participates:
                    qk_partial = q0 * kval0 + q1 * kval1 + q2 * kval2 + q3 * kval3
                    dprob_partial = do0 * vval0 + do1 * vval1 + do2 * vval2 + do3 * vval3
                score = utils.warp_reduce(qk_partial, lambda a, b: a + b, width=16)
                dprob = utils.warp_reduce(dprob_partial, lambda a, b: a + b, width=16)
                prob = Float32(0.0)
                ds_scaled = Float32(0.0)
                if lane == Int32(0) and participates:
                    prob = cute.math.exp2(score * scale_log2 - Float32(sRowLSE[row_i]), fastmath=True)
                    ds_scaled = prob * (dprob - Float32(sRowDelta[row_i])) * softmax_scale
                prob = utils.shuffle_sync(prob, Int32(0), width=16)
                ds_scaled = utils.shuffle_sync(ds_scaled, Int32(0), width=16)
                if participates:
                    if dim0 < mdKRows.shape[2]:
                        dk0 += ds_scaled * q0
                        dv0 += prob * do0
...
class FlashHSACachedGeneralizedFusedBwdRangeOwnedSm100:
    """Range-owned dK/dV backward over cached fused 8x8 exact+tail ranges."""

    arch = 100

    def __init__(self, *, local_k_chunk: int = 8):
        self.rows_per_range = 8
        self.keys_per_tile = 8
        self.max_local_k = 64
        self.local_k_chunk = int(local_k_chunk)
        self.num_threads = 32
    ...
    def __call__(
        ...
        mQLength: cute.Tensor,
        mExactTilePtr: cute.Tensor,
        mExactTileLocalKIdx: cute.Tensor,
        mTailTilePtr: cute.Tensor,
        mTailTileLocalKIdx: cute.Tensor,
        mTailMaskWords: cute.Tensor,
        mRangeLocalKPtr: cute.Tensor,
        mRangeLocalKRowIdx: cute.Tensor,
        softmax_scale: Float32,
        mdKRows: cute.Tensor,
        mdVRows: cute.Tensor,
        stream: cuda.CUstream,
        ...
        grid_x = mQRowIdx.shape[0]
        grid_y = mQRows.shape[1]
        grid_z = (self.max_local_k + self.local_k_chunk - 1) // self.local_k_chunk
        self.kernel(
            mQRows,
            mKRows,
            mVRows,
    ...
    def kernel(
        ...
        tidx, _, _ = cute.arch.thread_idx()
        range_idx, head_idx, chunk_idx = cute.arch.block_idx()
        lane = tidx % cute.arch.WARP_SIZE
        dim0 = lane
        dim1 = lane + cute.arch.WARP_SIZE
        q_length = Int32(mQLength[range_idx])
        local_k_begin = Int32(mRangeLocalKPtr[range_idx])
        local_k_end = Int32(mRangeLocalKPtr[range_idx + Int32(1)])
        local_k_count = local_k_end - local_k_begin
        chunk_local_k_begin = Int32(chunk_idx) * Int32(self.local_k_chunk)
        chunk_local_k_end = chunk_local_k_begin + Int32(self.local_k_chunk)
        if chunk_local_k_end > local_k_count:
            chunk_local_k_end = local_k_count
        chunk_local_k_count = chunk_local_k_end - chunk_local_k_begin
        smem = cutlass.utils.SmemAllocator()
        sQ = smem.allocate_tensor(
            mQRows.element_type,
            cute.make_layout((self.rows_per_range, 64)),
        ...
        sRowLSE = smem.allocate_tensor(cutlass.Float32, cute.make_layout((self.rows_per_range,)), byte_alignment=16)
        sRowValid = smem.allocate_tensor(cutlass.Int32, cute.make_layout((self.rows_per_range,)), byte_alignment=4)
        sLocalKRowIdx = smem.allocate_tensor(cutlass.Int32, cute.make_layout((self.local_k_chunk,)), byte_alignment=4)
        sDK = smem.allocate_tensor(cutlass.Float32, cute.make_layout((self.local_k_chunk, 64)), byte_alignment=16)
        sDV = smem.allocate_tensor(cutlass.Float32, cute.make_layout((self.local_k_chunk, 64)), byte_alignment=16)

        for elem_idx in cutlass.range(lane, Int32(self.rows_per_range) * Int32(64), cute.arch.WARP_SIZE, unroll=1):
            row_i = elem_idx // Int32(64)
            dim_idx = elem_idx - row_i * Int32(64)
            global_q_row = Int32(-1)
            if row_i < q_length:
        ...
        if lane < Int32(self.rows_per_range):
            ...
            sRowValid[row_i] = row_valid
        for local_idx in cutlass.range(lane, Int32(self.local_k_chunk), cute.arch.WARP_SIZE, unroll=1):
            global_local_k_idx = chunk_local_k_begin + local_idx
            if global_local_k_idx < local_k_count:
                sLocalKRowIdx[local_idx] = Int32(mRangeLocalKRowIdx[local_k_begin + global_local_k_idx])
            else:
                sLocalKRowIdx[local_idx] = Int32(-1)
        for elem_idx in cutlass.range(lane, chunk_local_k_count * Int32(64), cute.arch.WARP_SIZE, unroll=1):
            local_idx = elem_idx // Int32(64)
            dim_idx = elem_idx - local_idx * Int32(64)
            sDK[local_idx, dim_idx] = Float32(0.0)
            sDV[local_idx, dim_idx] = Float32(0.0)
        cute.arch.barrier()

        exact_tile_begin = Int32(mExactTilePtr[range_idx])
        exact_tile_end = Int32(mExactTilePtr[range_idx + Int32(1)])
        for tile_idx in cutlass.range(exact_tile_begin, exact_tile_end, Int32(1), unroll=1):
            for tile_col in range(self.keys_per_tile):
                local_k_idx = Int32(mExactTileLocalKIdx[tile_idx, tile_col])
                local_k_valid = (
                    local_k_idx >= chunk_local_k_begin
                    and local_k_idx < chunk_local_k_end
                )
                local_k_slot = local_k_idx - chunk_local_k_begin
                global_k_row = Int32(sLocalKRowIdx[local_k_slot]) if local_k_valid else Int32(-1)
                k0 = Float32(0.0)
                k1 = Float32(0.0)
                v0 = Float32(0.0)
                v1 = Float32(0.0)
                if global_k_row >= Int32(0):
                    if dim0 < mKRows.shape[2]:
                        k0 = Float32(mKRows[global_k_row, head_idx, dim0])
                        v0 = Float32(mVRows[global_k_row, head_idx, dim0])
                    if dim1 < mKRows.shape[2]:
                        k1 = Float32(mKRows[global_k_row, head_idx, dim1])
                        v1 = Float32(mVRows[global_k_row, head_idx, dim1])
                for row_idx in range(self.rows_per_range):
                    row_i = Int32(row_idx)
                    participates = row_i < q_length and sRowValid[row_i] != Int32(0)
                    q0 = Float32(sQ[row_i, dim0]) if participates and dim0 < mQRows.shape[2] else Float32(0.0)
                    q1 = Float32(sQ[row_i, dim1]) if participates and dim1 < mQRows.shape[2] else Float32(0.0)
                    do0 = Float32(sdO[row_i, dim0]) if participates and dim0 < mdORows.shape[2] else Float32(0.0)
                    do1 = Float32(sdO[row_i, dim1]) if participates and dim1 < mdORows.shape[2] else Float32(0.0)
                    qk_partial = Float32(0.0)
                    dprob_partial = Float32(0.0)
                    if participates:
                        qk_partial = q0 * k0 + q1 * k1
                        dprob_partial = do0 * v0 + do1 * v1
                    score = utils.warp_reduce(qk_partial, lambda a, b: a + b)
                    dprob = utils.warp_reduce(dprob_partial, lambda a, b: a + b)
                    prob = Float32(0.0)
                    ds_scaled = Float32(0.0)
                    if lane == Int32(0) and participates:
                        prob = cute.math.exp2(score * softmax_scale * Float32(_LOG2_E) - Float32(sRowLSE[row_i]), fastmath=True)
                        ds_scaled = prob * (dprob - Float32(sRowDelta[row_i])) * softmax_scale
                    prob = utils.shuffle_sync(prob, Int32(0))
                    ds_scaled = utils.shuffle_sync(ds_scaled, Int32(0))
                    if local_k_valid and participates:
                        if dim0 < mdKRows.shape[2]:
                            sDK[local_k_slot, dim0] += ds_scaled * q0
                            sDV[local_k_slot, dim0] += prob * do0
                        if dim1 < mdKRows.shape[2]:
                            sDK[local_k_slot, dim1] += ds_scaled * q1
                            sDV[local_k_slot, dim1] += prob * do1
                cute.arch.barrier()
        ...
        for tile_idx in cutlass.range(tail_tile_begin, tail_tile_end, Int32(1), unroll=1):
            for tile_col in range(self.keys_per_tile):
                local_k_idx = Int32(mTailTileLocalKIdx[tile_idx, tile_col])
                local_k_valid = (
                    local_k_idx >= chunk_local_k_begin
                    and local_k_idx < chunk_local_k_end
                )
                ...
                for row_idx in range(self.rows_per_range):
                    ...
                    if participates:
                        tail_mask_word = Int32(mTailMaskWords[tile_idx, row_i, Int32(0)])
                        participates = (tail_mask_word & (Int32(1) << Int32(tile_col))) != Int32(0)
                    q0 = Float32(sQ[row_i, dim0]) if participates and dim0 < mQRows.shape[2] else Float32(0.0)
                    q1 = Float32(sQ[row_i, dim1]) if participates and dim1 < mQRows.shape[2] else Float32(0.0)
                    do0 = Float32(sdO[row_i, dim0]) if participates and dim0 < mdORows.shape[2] else Float32(0.0)
                    do1 = Float32(sdO[row_i, dim1]) if participates and dim1 < mdORows.shape[2] else Float32(0.0)
        ...
        for elem_idx in cutlass.range(lane, chunk_local_k_count * Int32(64), cute.arch.WARP_SIZE, unroll=1):
            local_idx = elem_idx // Int32(64)
            dim_idx = elem_idx - local_idx * Int32(64)
            global_k_row = Int32(sLocalKRowIdx[local_idx])
            if global_k_row >= Int32(0):
                utils.atomic_add_fp32(
                    Float32(sDK[local_idx, dim_idx]),
                    utils.elem_pointer(mdKRows, (global_k_row, head_idx, dim_idx)),
                )
                utils.atomic_add_fp32(
                    Float32(sDV[local_idx, dim_idx]),
                    utils.elem_pointer(mdVRows, (global_k_row, head_idx, dim_idx)),
                )


def _run_cached_generalized_fused_bwd_dq_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    out_rows: torch.Tensor,
    ...
    q_row_idx: torch.Tensor,
    q_length: torch.Tensor,
    exact_tile_ptr: torch.Tensor,
    exact_k_row_idx: torch.Tensor,
    tail_tile_ptr: torch.Tensor,
    tail_k_row_idx: torch.Tensor,
    tail_mask_words: torch.Tensor,
    dq_rows: torch.Tensor,
    ...
    *,
    ...
    if compile_key not in _run_cached_generalized_fused_bwd_dq_kernel.compile_cache:
        kernel = FlashHSACachedGeneralizedFusedBwdDQSm100()
        _run_cached_generalized_fused_bwd_dq_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
            ...
            to_cute_tensor(q_row_idx, assumed_align=4),
            to_cute_tensor(q_length, assumed_align=4, leading_dim=0),
            to_cute_tensor(exact_tile_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(exact_k_row_idx, assumed_align=4),
            to_cute_tensor(tail_tile_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(tail_k_row_idx, assumed_align=4),
            to_cute_tensor(tail_mask_words, assumed_align=4, leading_dim=2),
            Float32(softmax_scale),
    ...
    _run_cached_generalized_fused_bwd_dq_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        out_rows,
...
_run_cached_generalized_fused_bwd_dq_kernel.compile_cache = get_jit_cache(
    "hsa_cached_generalized_fused_bwd_dq"
)


def _run_cached_generalized_fused_bwd_dkdv_range_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    out_rows: torch.Tensor,
    ...
    q_length: torch.Tensor,
    exact_tile_ptr: torch.Tensor,
    exact_tile_local_k_idx: torch.Tensor,
    tail_tile_ptr: torch.Tensor,
    tail_tile_local_k_idx: torch.Tensor,
    tail_mask_words: torch.Tensor,
    range_local_k_ptr: torch.Tensor,
    range_local_k_row_idx: torch.Tensor,
    dk_rows: torch.Tensor,
    dv_rows: torch.Tensor,
    *,
    softmax_scale: float,
    ...
    _require_cute_runtime()
    local_k_chunk = int(local_k_chunk)
    if local_k_chunk <= 0 or local_k_chunk > 64:
        raise ValueError(f"local_k_chunk must be in [1, 64], got {local_k_chunk}")
    compile_key = (
        "cached_generalized_fused_bwd_dkdv_range_v2",
        local_k_chunk,
        q_rows.dtype,
        k_rows.dtype,
        v_rows.dtype,
        ...
        lse_rows.dtype,
        q_row_idx.shape[0],
        exact_tile_local_k_idx.shape[0],
        tail_tile_local_k_idx.shape[0],
        range_local_k_row_idx.shape[0],
        q_rows.shape[1],
        q_rows.shape[2],
        v_rows.shape[2],
        torch.cuda.get_device_capability(q_rows.device),
    )
    if compile_key not in _run_cached_generalized_fused_bwd_dkdv_range_kernel.compile_cache:
        kernel = FlashHSACachedGeneralizedFusedBwdRangeOwnedSm100(local_k_chunk=local_k_chunk)
        _run_cached_generalized_fused_bwd_dkdv_range_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
            ...
            to_cute_tensor(q_length, assumed_align=4, leading_dim=0),
            to_cute_tensor(exact_tile_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(exact_tile_local_k_idx, assumed_align=4),
            to_cute_tensor(tail_tile_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(tail_tile_local_k_idx, assumed_align=4),
            to_cute_tensor(tail_mask_words, assumed_align=4, leading_dim=2),
            to_cute_tensor(range_local_k_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(range_local_k_row_idx, assumed_align=4, leading_dim=0),
            Float32(softmax_scale),
            to_cute_tensor(dk_rows, assumed_align=4),
            to_cute_tensor(dv_rows, assumed_align=4),
            cuda.CUstream(torch.cuda.current_stream().cuda_stream),
    ...
    _run_cached_generalized_fused_bwd_dkdv_range_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        out_rows,
        ...
        q_length,
        exact_tile_ptr,
        exact_tile_local_k_idx,
        tail_tile_ptr,
        tail_tile_local_k_idx,
        tail_mask_words,
        range_local_k_ptr,
        range_local_k_row_idx,
        Float32(softmax_scale),
        dk_rows,
        dv_rows,
        cuda.CUstream(torch.cuda.current_stream().cuda_stream),
...
_run_cached_generalized_fused_bwd_dkdv_range_kernel.compile_cache = get_jit_cache(
    "hsa_cached_generalized_fused_bwd_dkdv_range"
)


def _run_cached_generalized_fused_bwd_dkdv_kernel(
    q_rows: torch.Tensor,
    k_rows: torch.Tensor,
    v_rows: torch.Tensor,
    out_rows: torch.Tensor,
    ...
    q_length: torch.Tensor,
    tail_mask_words: torch.Tensor,
    owned_k_row_idx: torch.Tensor,
    owned_occurrence_ptr: torch.Tensor,
    owned_occurrence_kind: torch.Tensor,
    ...
    *,
    ...
    _require_cute_runtime()
    compile_key = (
        "cached_generalized_fused_bwd_dkdv_v2",
        q_rows.dtype,
        k_rows.dtype,
        v_rows.dtype,
        out_rows.dtype,
        ...
        lse_rows.dtype,
        q_row_idx.shape[0],
        owned_k_row_idx.shape[0],
        owned_occurrence_kind.shape[0],
        q_rows.shape[1],
        q_rows.shape[2],
        v_rows.shape[2],
        torch.cuda.get_device_capability(q_rows.device),
    )
    if compile_key not in _run_cached_generalized_fused_bwd_dkdv_kernel.compile_cache:
        kernel = FlashHSACachedGeneralizedFusedBwdKeyOwnedSm100()
        _run_cached_generalized_fused_bwd_dkdv_kernel.compile_cache[compile_key] = cute.compile(
            kernel,
            to_cute_tensor(q_rows),
            to_cute_tensor(k_rows),
            to_cute_tensor(v_rows),
            ...
            to_cute_tensor(q_length, assumed_align=4, leading_dim=0),
            to_cute_tensor(tail_mask_words, assumed_align=4, leading_dim=2),
            to_cute_tensor(owned_k_row_idx, assumed_align=4, leading_dim=0),
            to_cute_tensor(owned_occurrence_ptr, assumed_align=4, leading_dim=0),
            to_cute_tensor(owned_occurrence_kind, assumed_align=4, leading_dim=0),
            to_cute_tensor(owned_occurrence_range_idx, assumed_align=4, leading_dim=0),
            to_cute_tensor(owned_occurrence_tile_idx, assumed_align=4, leading_dim=0),
            to_cute_tensor(owned_occurrence_col_idx, assumed_align=4, leading_dim=0),
            Float32(softmax_scale),
            to_cute_tensor(dk_rows, assumed_align=4),
            to_cute_tensor(dv_rows, assumed_align=4),
            cuda.CUstream(torch.cuda.current_stream().cuda_stream),
    ...
    _run_cached_generalized_fused_bwd_dkdv_kernel.compile_cache[compile_key](
        q_rows,
        k_rows,
        v_rows,
        out_rows,
...
_run_cached_generalized_fused_bwd_dkdv_kernel.compile_cache = get_jit_cache(
    "hsa_cached_generalized_fused_bwd_dkdv"
)
...
def _run_direct_row_compact_bucket_forward(
    q_flat: torch.Tensor,
    k_flat: torch.Tensor,
    v_flat: torch.Tensor,
    bucket_plan,
    row_plan,
    direct_bucket_idx: int,
    out_rows: torch.Tensor,
    ...
    *,
    ...
    saved_packed_k_rows: torch.Tensor | None = None,
    saved_packed_v_rows: torch.Tensor | None = None,
    packed_k_rows: torch.Tensor | None = None,
    packed_v_rows: torch.Tensor | None = None,
) -> torch.Tensor | None:
    direct_bucket_packed_q = bucket_plan["bucket_packed_q"]
    direct_bucket_packed_k = bucket_plan["bucket_packed_k"]
    direct_bucket_q_row_range = bucket_plan["bucket_q_row_range"]
    direct_bucket_k_row_range = bucket_plan["bucket_k_row_range"]
    direct_bucket_q_length_range = bucket_plan["bucket_q_length_range"]
    direct_bucket_k_length_range = bucket_plan["bucket_k_length_range"]
    direct_bucket_q_row_idx = bucket_plan["bucket_q_row_idx"]
    direct_bucket_k_row_idx = bucket_plan["bucket_k_row_idx"]
    direct_bucket_q_length = bucket_plan["bucket_q_length"]
    direct_bucket_k_length = bucket_plan["bucket_k_length"]
    ...
    row_k_cap = int(row_bucket_row_k_cap[direct_bucket_idx])
    if int(direct_bucket_packed_q[direct_bucket_idx]) != 2 or row_k_cap <= 0:
        return None
    ...
    row_k_row_idx = row_bucket_row_k_row_idx[row_k_start:row_k_end].contiguous().view(bucket_size, 2, row_k_cap)
    row_k_to_union_idx = row_bucket_row_k_to_union_idx[row_k_to_union_start:row_k_to_union_end].contiguous().view(
        bucket_size, 2, row_k_cap
    )
    row_k_length = row_bucket_row_k_length[row_k_length_start:row_k_length_end].contiguous().view(bucket_size, 2)

    if packed_k_rows is not None and packed_v_rows is not None:
        _run_synthetic_direct_row_micro_fwd_packed_kv_kernel(
            q_flat,
            packed_k_rows,
            packed_v_rows,
            q_row_idx,
    ...
    elif saved_packed_k_rows is not None and saved_packed_v_rows is not None and saved_prob_rows is not None:
        _run_synthetic_direct_row_micro_fwd_save_packed_kv_kernel(
            q_flat,
            k_flat,
            v_flat,
            q_row_idx,
            ...
            out_rows,
            out_lse,
            saved_packed_k_rows[q_length_start:q_length_end, :packed_k].contiguous(),
            saved_packed_v_rows[q_length_start:q_length_end, :packed_k].contiguous(),
            saved_prob_rows[q_length_start:q_length_end],
            softmax_scale=softmax_scale,
        )
    elif saved_prob_rows is not None:
        _run_synthetic_direct_row_micro_fwd_save_prob_kernel(
            q_flat,
            k_flat,
            v_flat,
            q_row_idx,
    ...
    return q_row_idx.reshape(bucket_size * 2).contiguous()


def _run_union_row_compact_forward(
    q_flat: torch.Tensor,
    k_flat: torch.Tensor,
    v_flat: torch.Tensor,
    ...
    *,
    ...
    row_plan = None if union_plan is None else union_plan.get("row_compact_plan")
    if union_plan is None or row_plan is None:
        return False
    packed_union_kv_enabled = runtime is not None and _synthetic_sparse_parse_packed_kv_fwd_enabled()
    packed_union_workspace = None
    if packed_union_kv_enabled:
        workspace = runtime.synthetic_forward_workspace
        if workspace is None:
            workspace = {}
            runtime.synthetic_forward_workspace = workspace
        packed_union_workspace = workspace.setdefault("row_compact_packed_union_kv", {})

    def _get_packed_union_kv_buffers(bucket_size: int, packed_k: int) -> tuple[torch.Tensor, torch.Tensor]:
        assert packed_union_workspace is not None
        key = (
            str(k_flat.device),
            k_flat.dtype,
            v_flat.dtype,
        ...
        bufs = packed_union_workspace.get(key)
        needed = bucket_size * packed_k
        if bufs is None or bufs[0].shape[0] < needed:
            bufs = (
                torch.empty((needed, k_flat.shape[1], k_flat.shape[2]), dtype=k_flat.dtype, device=k_flat.device),
                torch.empty((needed, v_flat.shape[1], v_flat.shape[2]), dtype=v_flat.dtype, device=v_flat.device),
            )
            packed_union_workspace[key] = bufs
        return bufs[0][:needed], bufs[1][:needed]

    for bucket_idx in range(len(union_plan["bucket_packed_k"])):
        packed_k_rows = None
        packed_v_rows = None
        if packed_union_kv_enabled:
            packed_k = int(union_plan["bucket_packed_k"][bucket_idx])
            q_length_start, q_length_end = union_plan["bucket_q_length_range"][bucket_idx]
            k_row_start, k_row_end = union_plan["bucket_k_row_range"][bucket_idx]
            bucket_size = q_length_end - q_length_start
            union_k_row_idx_flat = union_plan["bucket_k_row_idx"][k_row_start:k_row_end].contiguous()
            packed_k_flat, packed_v_flat = _get_packed_union_kv_buffers(bucket_size, packed_k)
            _run_synthetic_pack_kv_rows_kernel(k_flat, v_flat, union_k_row_idx_flat, packed_k_flat, packed_v_flat)
            packed_k_rows = packed_k_flat.view(bucket_size, packed_k, k_flat.shape[1], k_flat.shape[2]).contiguous()
            packed_v_rows = packed_v_flat.view(bucket_size, packed_k, v_flat.shape[1], v_flat.shape[2]).contiguous()
        q_row_idx_flat = _run_direct_row_compact_bucket_forward(
            q_flat,
            k_flat,
            v_flat,
            union_plan,
            ...
            packed_v_rows=packed_v_rows,
        )
        if q_row_idx_flat is None:
            return False
    return True
...
def _run_direct_row_compact_forward(
    ...
    *,
    ...
    if _run_union_row_compact_forward(
        q_flat,
        k_flat,
        v_flat,
        total_out,
    ...
    saved_packed_k_rows = None
    saved_packed_v_rows = None
    save_saved_prob = runtime is not None and (
        _should_save_synthetic_bucket_dense_prob(q_flat, k_flat, v_flat)
        or _should_save_synthetic_bucket_dense_packed_qkv(q_flat, k_flat, v_flat)
    )
    multi_segment_direct = any(
        (segment_end - segment_start) > 1 for segment_start, segment_end in direct_qgroup_bucket_segment_range
    )
    if save_saved_prob and multi_segment_direct:
        return False
    if save_saved_prob:
        saved_prob_dtype = (
            torch.bfloat16 if _should_save_synthetic_bucket_dense_packed_qkv(q_flat, k_flat, v_flat) else torch.float32
        )
        saved_prob_rows = _get_bucket_dense_saved_prob_forward_buffer(
            runtime,
            device=q_flat.device,
            num_direct_qgroups=len(direct_bucket_q_length),
            num_heads=q_flat.shape[1],
            dtype=saved_prob_dtype,
        )
    if runtime is not None and _should_save_synthetic_bucket_dense_packed_qkv(q_flat, k_flat, v_flat):
        max_packed_k = max((int(x) for x in direct_bucket_packed_k), default=0)
        saved_packed_k_rows, saved_packed_v_rows = _get_bucket_dense_saved_packed_union_kv_forward_buffers(
            runtime,
            device=q_flat.device,
            num_direct_qgroups=len(direct_bucket_q_length),
            max_packed_k=max_packed_k,
            num_k_heads=k_flat.shape[1],
            head_dim_k=k_flat.shape[2],
            num_v_heads=v_flat.shape[1],
            head_dim_v=v_flat.shape[2],
        )

    def _get_temp_buffers():
        if runtime is None:
            raise RuntimeError("Segmented row-compact forward requires runtime workspace")
        workspace = runtime.synthetic_forward_workspace
        if workspace is None:
            workspace = {}
            runtime.synthetic_forward_workspace = workspace
        row_compact_workspace = workspace.setdefault("direct_row_compact", {})
        key = (
            str(q_flat.device),
            total_out.dtype,
            total_lse.dtype,
        ...
        bufs = row_compact_workspace.get(key)
        if bufs is None:
            bufs = (torch.empty_like(total_out), torch.empty_like(total_lse))
            row_compact_workspace[key] = bufs
        return bufs
    ...
    for qgroup_bucket_idx in range(len(direct_qgroup_bucket_segment_range)):
        ...
        if len(direct_bucket_ids) == 1:
            ...
            q_row_idx_flat = _run_direct_row_compact_bucket_forward(
                ...
                saved_packed_k_rows=saved_packed_k_rows,
                saved_packed_v_rows=saved_packed_v_rows,
            )
            if q_row_idx_flat is None:
                return False
            continue
    ...
    *,
    ...
    if save_packed_qkv:
        (
            ...
            runtime,
            device=q_flat.device,
            direct_bucket_size=direct_bucket_size,
            direct_bucket_packed_q=direct_bucket_packed_q,
            direct_bucket_packed_k=direct_bucket_packed_k,
            num_q_heads=q_flat.shape[1],
            head_dim_q=q_flat.shape[2],
            num_k_heads=k_flat.shape[1],
            head_dim_k=k_flat.shape[2],
            num_v_heads=v_flat.shape[1],
            head_dim_v=v_flat.shape[2],
    ...
    for qgroup_bucket_idx in range(len(direct_qgroup_bucket_segment_range)):
        ...
        if not direct_bucket_ids:
            continue
        if not save_packed_qkv and len(direct_bucket_ids) == 1:
            direct_bucket_idx = int(direct_bucket_ids[0])
            ...
            k_row_idx_flat = k_row_idx.reshape(bucket_size_value * packed_k).contiguous()
            q_length = direct_bucket_q_length[q_length_start:q_length_end].contiguous()
            k_length = direct_bucket_k_length[k_length_start:k_length_end].contiguous()
            if save_packed_qkv:
                assert packed_saved_q is not None
                assert packed_saved_k is not None
                assert packed_saved_v is not None
                assert packed_q_offsets is not None
                assert packed_kv_offsets is not None
                q_buf_flat = packed_saved_q[
                    packed_q_offsets[direct_bucket_idx] : packed_q_offsets[direct_bucket_idx + 1]
                ]
                k_buf_flat = packed_saved_k[
                    packed_kv_offsets[direct_bucket_idx] : packed_kv_offsets[direct_bucket_idx + 1]
                ]
                v_buf_flat = packed_saved_v[
                    packed_kv_offsets[direct_bucket_idx] : packed_kv_offsets[direct_bucket_idx + 1]
                ]
                _run_synthetic_pack_rows_kernel(q_flat, q_row_idx_flat, q_buf_flat)
                _run_synthetic_pack_kv_rows_kernel(k_flat, v_flat, k_row_idx_flat, k_buf_flat, v_buf_flat)
...
def _run_direct_segmented_backward_saved_packed(
    q_flat: torch.Tensor,
    k_flat: torch.Tensor,
    v_flat: torch.Tensor,
    ...
    metadata,
    execution_plan,
    packed_q_rows: torch.Tensor,
    packed_k_rows: torch.Tensor,
    packed_v_rows: torch.Tensor,
    packed_q_offsets: list[int],
    packed_kv_offsets: list[int],
    ...
    *,
    ...
    for direct_bucket_idx in range(len(direct_bucket_size)):
        ...
        q_length = direct_bucket_q_length[q_length_start:q_length_end].contiguous()
        k_length = direct_bucket_k_length[k_length_start:k_length_end].contiguous()
        q_buf_flat = packed_q_rows[packed_q_offsets[direct_bucket_idx] : packed_q_offsets[direct_bucket_idx + 1]]
        k_buf_flat = packed_k_rows[packed_kv_offsets[direct_bucket_idx] : packed_kv_offsets[direct_bucket_idx + 1]]
        v_buf_flat = packed_v_rows[packed_kv_offsets[direct_bucket_idx] : packed_kv_offsets[direct_bucket_idx + 1]]
        if bool(direct_bucket_dense[direct_bucket_idx]):
            _run_synthetic_direct_micro_bwd_dense_packed_input_kernel(
                q_buf_flat,
                k_buf_flat,
                v_buf_flat,
                out_flat,
            ...
            _run_synthetic_direct_micro_bwd_masked_packed_input_kernel(
                q_buf_flat,
                k_buf_flat,
                v_buf_flat,
                out_flat,
...
def _run_bucketed_forward(
    ...
    *,
    ...
    if (
        ...
        and _can_use_direct_forward_micro_runtime(metadata, q_flat, k_flat, v_flat)
    ):
    ...
    if not _load_hsa_module()._forward_execution_plan_has_bucket_execution(execution_plan):
        hsa_module = _load_hsa_module()
        metadata = hsa_module._ensure_hsa_synthetic_grid_metadata(
            schedule,
            runtime,
            require_full_forward_plan=True,
    ...
    for qgroup_bucket_idx, packed_q in enumerate(qgroup_bucket_packed_q):
        ...
        for split_bucket_idx in split_bucket_ids:
            ...
            if save_packed_qkv:
                ...
                assert packed_q_offsets is not None
                assert packed_kv_offsets is not None
                q_buf_flat = packed_saved_q[packed_q_offsets[split_bucket_idx] : packed_q_offsets[split_bucket_idx + 1]]
                if use_qgroup_q:
                    q_buf_flat.copy_(qgroup_q_buf_flat[: bucket_size * packed_q_bucket])
                else:
                ...
                k_buf_flat = packed_saved_k[
                    packed_kv_offsets[split_bucket_idx] : packed_kv_offsets[split_bucket_idx + 1]
                ]
                v_buf_flat = packed_saved_v[
                    packed_kv_offsets[split_bucket_idx] : packed_kv_offsets[split_bucket_idx + 1]
                ]
...
def run_hsa_fwd_sm100_synthetic_grid(
    ...
    *,
    ...
    runtime.synthetic_forward_prob_token = int(getattr(runtime, "synthetic_forward_prob_token", 0)) + 1
...
def run_hsa_bwd_sm100_synthetic_grid(
    ...
    hash_ids=None,
    *,
    forward_prob_token: int = 0,
    runtime=None,
):
    from flash_attn.cute.flash_hsa_bwd_sm100 import run_hsa_bwd_sm100_blocksparse

    runtime = _ensure_runtime(schedule, runtime, q, k)
    _load_hsa_module()._ensure_hsa_backward_synthetic_grid_metadata(schedule, runtime)
    metadata = runtime.forward_synthetic_grid
    if (
    ...
    saved_prob_cache = None
    saved_packed_union_kv_cache = None
    if variant in {"bucket_dense_saved_prob", "bucket_dense_saved_packed"}:
        saved_prob_cache = _get_bucket_dense_saved_prob_cache(
            runtime,
            device=q.device,
            num_direct_qgroups=len(direct_bucket_q_length),
            num_heads=q_flat.shape[1],
            token=forward_prob_token,
            dtype=torch.bfloat16 if variant == "bucket_dense_saved_packed" else torch.float32,
        )
    if variant == "bucket_dense_saved_packed":
        max_packed_k = max((int(x) for x in direct_bucket_packed_k), default=0)
        saved_packed_union_kv_cache = _get_bucket_dense_saved_packed_union_kv_cache(
            runtime,
            device=q.device,
            num_direct_qgroups=len(direct_bucket_q_length),
            max_packed_k=max_packed_k,
    ...
    if row_compact_active:
        ...
        for direct_bucket_idx, row_k_cap in enumerate(row_bucket_row_k_cap):
            ...
            if row_compact_bwd_mode == "one_kernel":
                bucket_saved_prob_rows = None if saved_prob_cache is None else saved_prob_cache[q_length_start:q_length_end]
                bucket_saved_packed_k_rows = None
                bucket_saved_packed_v_rows = None
                if saved_packed_union_kv_cache is not None:
                    packed_k_rows, packed_v_rows = saved_packed_union_kv_cache
                    bucket_saved_packed_k_rows = packed_k_rows[q_length_start:q_length_end, :packed_k].contiguous()
                    bucket_saved_packed_v_rows = packed_v_rows[q_length_start:q_length_end, :packed_k].contiguous()
                _run_synthetic_direct_row_micro_bwd_kernel_row_compact_one_kernel(
                    ...
                    dk_accum,
                    dv_accum,
                    saved_prob_rows=bucket_saved_prob_rows,
                    saved_packed_k_rows=bucket_saved_packed_k_rows,
                    saved_packed_v_rows=bucket_saved_packed_v_rows,
                    softmax_scale=softmax_scale,
                    max_unique_key_occurrences=max_unique_key_occurrences,
                    workspace=workspace,
                )
